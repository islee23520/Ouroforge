...
## Era P 2.5D migration on-ramp governance

Era P — 2.5D Migration On-Ramp (3D presentation over 2D-deterministic logic) — is recorded as **complete on merged evidence** for Milestones 96-99 plus the M100 governance refresh. The completed evidence chain covers the M96 parent 2.5D scope/gate (#2191), M97 glTF geometry plus orthographic-camera presentation import (#2192-#2195 / Scenario Coverage v83), M98 billboard/sprite-stack/2D-in-3D presentation import (#2196-#2199 / Scenario Coverage v84), M99 2.5D verification with deterministic state-hash primary evidence and perceptual render secondary evidence (#2200-#2202 / Scenario Coverage v85), and this M100 governance refresh (#2203). The governance contract is recorded in [`docs/era-p-2-5d-migration-on-ramp-governance-refresh.md`](era-p-2-5d-migration-on-ramp-governance-refresh.md).

Era P remains one-way and presentation-only. glTF meshes, orthographic cameras, billboards, sprite stacks, 2D-in-3D planes, camera-facing rules, slice order, pivots, texture refs, alpha/sort metadata, and pixel-art filtering may become deterministic Ouroforge-native presentation evidence when backed by source-project/open-text provenance and honest fidelity reports. Gameplay logic, physics authority, animation events that mutate state, shader/VFX behavior, tacit feel, and any source-engine script behavior are not imported, translated, or called `ported`; they become clean-room re-derivation tasks with captured-oracle requirements. Deterministic state-hash evidence stays primary for authoritative 2.5D/3D state, perceptual render comparison (SSIM/pixel-diff) stays secondary for presentation corroboration, Studio owns no artifact semantics or trusted writes, no new data store is authorized, and #1/#23 remain open governance anchors.

## Era Q full-3D on-ramp re-evaluation governance

Era Q — Full-3D On-Ramp Re-evaluation — records the M101 decision as **DEFER on merged evidence** in [`docs/full-3d-on-ramp-reevaluation-gate-v1.md`](full-3d-on-ramp-reevaluation-gate-v1.md). The gate considered the completed Era O 2D source-only on-ramp, Era P 2.5D presentation import, and Era R clean-room re-derivation evidence, then deferred M102-M106 because full 3D would require a glTF 3D web runtime, deterministic 3D physics re-simulation, and a two-tier evidence model without enough demand evidence to outweigh moat dilution.

The decision is implementation-free. M102-M106 remain GO-gated and must not be created, built, or treated as authorized by this lane. If a future explicit GO is recorded, it must preserve one-way source-project/open-text import, clean-room re-derivation rather than translation, no finished-game auto-port, no live bridge, no embedded foreign runtime, no decompiled-code copying, deterministic state-hash primary evidence with perceptual render comparison secondary, honest Yellow/Red fidelity gaps, Rust-owned artifact semantics/gated writes, no trusted Studio writes, no new data store, and human Ring 2 fun/feel/release authority. #1/#23 remain open governance anchors.

## Era R semantic re-derivation governance

Era R — Interrogated Semantic Re-Derivation (legacy logic → verified deterministic behavior) — is recorded as **complete on merged evidence** for Milestones 107-113 plus the M114 governance refresh. The completed chain covers the M107 methodology/gate ADR (#2220), M108 legacy ingestion + behavioral-unit extraction (#2221-#2223 / Scenario Coverage v90), M109 interrogation + oracle capture (#2224-#2227 / Scenario Coverage v91), M110 deterministic re-expression (#2228-#2231 / Scenario Coverage v92), M111 differential verification A/B (#2232-#2234 / Scenario Coverage v93), M112 semantic-port coverage + convergence (#2235-#2236 / Scenario Coverage v94), M113 re-derivation UX + human intent/feel escalation (#2237-#2240 / Scenario Coverage v95), and this M114 governance refresh (#2241). The governance contract is recorded in [`docs/era-r-semantic-rederivation-governance-refresh.md`](era-r-semantic-rederivation-governance-refresh.md). #1 and #23 remain open.
...
Scenario Coverage v91 (`docs/scenario-coverage-v91-tacit-oracle-capture.md`) locks the M109 regression ledger for missing/incomplete oracles, blocked provenance, no port claims, deterministic state-hash drift, and the Rust data-plane / Studio presentation-only split.
M110 is scoped by `docs/deterministic-re-expression-engine-contract-v1.md`, which defines the deterministic re-expression gated path, exact Rust-owned inputs and outputs, conservative Green/Yellow/Red fidelity and oracle rules, 2D bit-exact / 2.5D-3D state-hash requirements, and the two-plane Studio no-trusted-write boundary.
The first M110 Rust data-plane implementation lives in `crates/ouroforge-core::deterministic_reexpression`; the demo (`docs/deterministic-reexpression-demo-v1.md`) records a captured unit re-expressed as a gated behavior draft, an oracle-less unit left as a Yellow task, stable state-hash digest behavior, and no auto-port claim.
Scenario Coverage v92 (`docs/scenario-coverage-v92-deterministic-reexpression.md`) locks the M110 regression ledger for lossy imports not grading clean, ungated auto-translation/port claims failing Red, deterministic state-hash drift, 3D state-hash-primary/render-secondary verification, and the Rust data-plane / Studio presentation-only split.
M111 is scoped by `docs/differential-verification-behavioral-ab-contract-v1.md`, which defines behavioral A/B verification over captured oracles and Ouroforge-native re-expression candidates, the preflight gates for source-only clean-room provenance and source-apply handoff, conservative Green/Yellow/Red fidelity/oracle rules, deterministic 2D and 2.5D/3D state-hash requirements, and the hand-off to M112 semantic-port coverage.
The first M111 Rust data-plane implementation lives in `crates/ouroforge-core::differential_verification` and compares captured oracle outcomes against native observations, emitting explicit fidelity gaps, rollback tasks, deterministic digests, and M112 handoffs only after passing evidence.
Scenario Coverage v93 (`docs/scenario-coverage-v93-differential-verification.md`) locks the M111 regression ledger for no auto-port without oracle, lossy outcome mismatches not grading clean, ungated/auto-translated ports failing preflight, deterministic state-hash breaks failing Red, 3D render evidence as secondary, and the Rust data-plane / Studio presentation-only split.
M112 is scoped by `docs/semantic-port-coverage-convergence-v1.md`, which defines the Rust-owned semantic-port coverage ledger, residual backlog, convergence statuses, Ring 2 human-feel escalation, deterministic evidence requirements, and the no-fully-ported-claim boundary.
Scenario Coverage v94 (`docs/scenario-coverage-v94-semantic-port-coverage.md`) locks the M112 regression ledger for no auto-port without oracle, lossy imports not grading clean, ungated/decompiled/runtime refs failing closed, deterministic state-hash evidence, 3D render evidence as secondary, and the Rust data-plane / Studio presentation-only split.
M113 is scoped by `docs/rederivation-ux-human-intent-feel-escalation-contract-v1.md`, which defines the re-derivation UX read model, human intent/feel escalation path, fidelity/oracle display rules, gated CLI/review-apply writes, Ring 2 boundaries, and the no Studio trusted-write contract.
The M113 demo (`docs/rederivation-ux-demo-v1.md`) records a runnable Studio-side evidence fixture and summary for re-derivation UX, proving no auto-port claim, oracle-gated Green behavior, human-owned intent/feel escalation, deterministic state-hash primary evidence, and no Studio trusted writes.
Scenario Coverage v95 (`docs/scenario-coverage-v95-rederivation-ux.md`) locks the M113 regression ledger for honest fidelity UX, no auto-port without oracle, lossy imports not grading clean, ungated/trusted-write drift failing closed, deterministic state-hash evidence, and Ring 2 intent/feel escalation through gated CLI previews.


Era N is recorded complete on merged evidence for Human-Grade Studio and Adoption UX. The chain covers M82 non-developer generative front door (#2078-#2081 / Scenario Coverage v72), M83 onboarding/templates/docs (#2082-#2085 / Scenario Coverage v73), M84 accessibility/i18n/themes/keyboard (#2086-#2089 / Scenario Coverage v74), M85 Human-Grade Studio (#2090-#2094 / Scenario Coverage v75), M86 local Studio packaging/delivery (#2095-#2098 / Scenario Coverage v76), and M87 governance/adoption assessment (#2099 / Scenario Coverage v77). Era N improves newcomer time-to-first-verified-game structurally through guided intake, templates, first-run docs, accessibility, a unified local Studio, and local packaging smoke evidence. It does not introduce a no-code product claim, hosted/multi-user Studio, command bridge, raw bypass, direct Elixir artifact write, new data store, release/deploy/signing path, mandatory human dependency, fun/taste automation, or release go/no-go automation. Rust remains the data plane; Elixir/OTP + Phoenix LiveView remains local control + presentation; Studio remains read + gated-write and local-first; hosted collaboration remains Layer-3 DEFER. #1 and #23 remain open.

...
Era P — 2.5D Migration On-Ramp (3D presentation over 2D-deterministic logic) — is in progress on merged evidence. M96 established the parent 2.5D on-ramp contract, M97 established and implemented glTF geometry plus orthographic-camera presentation import, and M98 is scoped by [`docs/billboard-sprite-stack-presentation-contract-v1.md`](billboard-sprite-stack-presentation-contract-v1.md). The M98 contract defines the source-project/open-text subset for billboards, sprite stacks, and 2D-in-3D planes; exact Rust-owned outputs; conservative 🟢/🟡/🔴 fidelity grades; oracle/no-port rules; and the Era R hand-off for behavior-bearing units.

Era P remains one-way and presentation-only. Billboards, sprite stacks, 2D-in-3D planes, camera-facing rules, slice order, pivots, texture refs, alpha/sort metadata, and pixel-art filtering may become deterministic Ouroforge-native presentation evidence. Gameplay logic, physics authority, animation events that mutate state, shader/VFX behavior, tacit feel, and any source-engine script behavior are not imported or translated; they become clean-room re-derivation tasks with captured-oracle requirements. Deterministic state-hash evidence stays primary for authoritative state, perceptual render evidence stays secondary for 2.5D presentation, Studio owns no artifact semantics or trusted writes, and #1/#23 remain open governance anchors.

...
source file class validation
  ...
  -> future allowlist-only sandbox dry-run evidence
...
4. **Test Command Allowlist** — define the narrow local command vocabulary that
   ...
   execution surface.
5. **Sandbox Dry-Run Evaluator** — future scoped only. If implemented later, it
   must use an isolated generated sandbox/worktree, the test command allowlist,
   explicit operator invocation, and generated evidence. It must not mutate the
   trusted main worktree or claim secure sandboxing.
...
- optionally, in a later slice, evaluate a preview in an isolated generated
  sandbox/worktree through explicit Rust-owned allowlisted dry-run commands.
...
Sandbox dry-run evaluation is future scoped and must remain allowlist-only. A
future dry-run evaluator must:
...
- run only explicit allowlisted commands through Rust-owned CLI code;
...
This scope issue does not implement a runner.
...
- optional sandbox dry-run evidence when generated by a future Rust command.
...
- generated-state audit proving `runs/`, `target/`, dashboard exports,
  sandboxes, and local tool state remain ignored/untracked;
...
- browser/dashboard/Studio surfaces are constrained to read-only preview
  inspection;
- sandbox dry-run is future scoped, isolated, and allowlist-only;

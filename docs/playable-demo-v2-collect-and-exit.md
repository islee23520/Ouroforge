...
Rust-trusted, and browser-observable.
...
- `scenes/collect-and-exit.scene.json` contains the player, floor, key trigger,
  gated door/exit trigger, HUD values, animation metadata, and audio intent.
...
Generated run/dashboard evidence remains local and must not be committed. If a
manual run is used for review, record the generated run id in the PR or issue
...
export, plugin runtime, hosted services, production-grade authoring tool behavior, marketplace
features, visual scripting, or replacement of an existing general-purpose game
engine.

...
> Status: pre-release private MVP moving toward public open-source readiness. Ouroforge is not a Godot replacement, does not promise compatibility, and should be treated as an inspectable local prototype. Current roadmap docs record completed Evidence Fidelity & Trust Boundary hardening, Agentic Review & Regression Promotion v1, and Agentic Loop Orchestration v1 while preserving a local-first, Rust-trusted, browser-read-only boundary.
...
- Inspect Project Workspace Loop v1, Evidence Fidelity, Agentic Review/Regression, and Agentic Loop Orchestration state in the static Studio cockpit without browser writes or command execution.
...
- `docs/` — architecture, roadmap, evidence-fidelity/trust-boundary contracts, manifest/scaffold/scenario-pack/project-run/project-comparison/project-mutation contracts, source mutation preview scope, artifact write policy, public-readiness audit, and demo evidence notes.

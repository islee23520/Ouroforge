...
> Status: pre-release private MVP moving toward public open-source readiness. Ouroforge is not a Godot replacement, does not promise compatibility, and should be treated as an inspectable local prototype. Current roadmap docs record completed Evidence Fidelity & Trust Boundary hardening and Agentic Review & Regression Promotion v1 while preserving a local-first, Rust-trusted, browser-read-only boundary.
...
- Inspect Project Workspace Loop v1, Evidence Fidelity, and Agentic Review/Regression state in the static Studio cockpit without browser writes or command execution.

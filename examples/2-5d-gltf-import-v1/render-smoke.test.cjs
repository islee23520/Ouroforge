...
function renderPresentation(scene) {
  ...
  const presentationLayers = scene.presentationLayers
    .map((layer) => ({
      id: layer.id,
      kind: layer.kind,
      sort: layer.sortingKey,
      stateAuthority: layer.authority
    }))
  ...
  return {
    ...
    boundary: 'Perceptual render smoke only; Rust-owned stateHashPrimary remains authoritative; M98 presentation layers cannot mutate deterministic logic/evidence.'
  };
}

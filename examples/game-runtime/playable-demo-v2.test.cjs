...
(async () => {
  ...
  assert.equal(state.renderQueue.schemaVersion, 'ouroforge.scene-render-queue.v1');
  assert.equal(state.renderQueue.frameId, 'tick-0');
  assert.equal(state.renderQueue.sceneId, state.sceneId);
  assert.equal(state.renderQueue.validation.status, 'ready');
  assert.ok(state.renderQueue.renderables.some((renderable) => renderable.sourceKind === 'tilemap-layer'));
  assert.ok(state.renderQueue.renderables.some((renderable) => renderable.sourceKind === 'entity' && renderable.sourceId === 'player'));
  assert.deepEqual(Array.from(state.renderQueue.readOnlyInspection.disallowedActions), ['trusted writes', 'command bridge', 'live mutation']);
  const frameStats = api.getFrameStats();
  assert.equal(frameStats.renderBreakdownFrameId, state.renderBreakdown.frameId);
  assert.equal(frameStats.renderQueueFrameId, state.renderQueue.frameId);
  assert.equal(frameStats.renderQueueLayerCount, state.renderQueue.layers.length);

...
assert.equal(state.runtimeState.schemaVersion, 'runtime-state-read-model-v1');
assert.equal(state.runtimeState.authority, 'browser_runtime_evidence_input_not_trusted_persistence');
assert.match(state.runtimeState.digest.value, /^[0-9a-f]{16}$/);
assert.ok(state.runtimeEvents.some((event) => event.type === 'runtime.save.created'));
assert.ok(state.runtimeEvents.some((event) => event.type === 'runtime.save.loaded'));
player = state.entities.find((entity) => entity.id === 'player');

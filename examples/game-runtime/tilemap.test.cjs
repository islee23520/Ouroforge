...
const tileFrameStats = api.getFrameStats();
assert.equal(tileFrameStats.tilemapRenderLayerCount, 1);
assert.equal(tileFrameStats.tilemapRenderCellCount, 2);
assert.equal(tileFrameStats.tilemapRenderDrawnTileCount, 2);
assert.equal(tileFrameStats.tilemapRenderMissingTileRefCount, 0);
state = api.step(1);

...
assert.match(roadmapDoc, /After #608, the recommended next milestone candidate is Gameplay Scripting \/\s+Logic System v1 \(#611-#625\)/);
assert.match(roadmapDoc, /must not become arbitrary third-party\s+code loading, plugin runtime, browser command bridge, hosted execution,\s+production scripting, source-apply authority, or Godot replacement scope/);

...
const provenanceAuditMarkup = cockpit.renderProvenanceAuditPanel(provenanceAuditRun);
...
const provenanceAuditGovernance = {
  ...
  wording: 'Read-only Studio audit panel for exported JSON; no production-ready engine, Godot replacement, auto-approval, or browser trusted-write claim.',
  compatibility: 'Existing cockpit contracts still render while provenance_audit/provenanceAudit/provenance_bundle/provenanceBundle are accepted as optional read models.',
  anchors: ['#1 remains open', '#23 remains open'],
};
assert.match(provenanceAuditGovernance.generatedState, /fixture-scoped/);
assert.match(provenanceAuditGovernance.wording, /Read-only Studio audit panel/);
...
assert.match(provenanceAuditGovernance.compatibility, /Existing cockpit contracts/);
...
assert.match(roadmapDoc, /Full Studio Editor v1 \(#757-#776\) is now complete as a local-first,\s+evidence-backed Studio authoring cockpit foundation under #1/);
assert.match(roadmapDoc, /project overview, scene tree,\s+entity\/component inspector, visual scene canvas, draft-only authoring model/);
assert.match(roadmapDoc, /Safe Source Apply handoff preview, asset browser\/metadata inspection,\s+scenario\/evidence and evidence-timeline panels, export\/package inspection/);
assert.match(roadmapDoc, /workspace persistence, command palette,\s+accessibility\/performance\/diagnostics coverage, fixture-scoped integrated demo,\s+Scenario Coverage v17/);
assert.match(roadmapDoc, /does not authorize direct Studio trusted\s+writes, browser command bridges, arbitrary shell execution/);
assert.match(roadmapDoc, /native desktop\s+editor, advanced visual scripting, full asset import pipeline, timeline or\s+animation editor parity, tilemap\/terrain editor parity, production collaboration/);
assert.match(roadmapDoc, /#1 remains open\. #23 remains open/);
assert.match(roadmapDoc, /Treat Full Studio Editor v1 as complete but bounded/);
assert.doesNotMatch(roadmapDoc, /production-ready editor is available|current Godot replacement is implemented|full Godot editor parity is implemented|secure sandbox is guaranteed|browser trusted writes are enabled|command bridge is enabled|auto-apply enabled|auto-merge enabled|plugin runtime enabled|marketplace behavior enabled/);
...
const sourceApplyHandoffModel = cockpit.studioSourceApplyHandoffModel(cockpit.studioDraftAuthoringState(run).drafts[0], run);
assert.strictEqual(sourceApplyHandoffModel.review_required, true);
assert.strictEqual(sourceApplyHandoffModel.rollback_required, true);
assert.strictEqual(sourceApplyHandoffModel.evidence_required, true);
assert.strictEqual(sourceApplyHandoffModel.apply_capability, false);
assert.ok(sourceApplyHandoffModel.forbidden_actions.includes('apply_patch'));
assert.ok(sourceApplyHandoffModel.forbidden_actions.includes('merge_branch'));
assert.ok(sourceApplyHandoffModel.forbidden_actions.includes('self_approve_review'));
assert.ok(sourceApplyHandoffModel.forbidden_actions.includes('bypass_review_gate'));
const sourceApplyHandoffMarkup = cockpit.renderStudioSourceApplyHandoffSurface(run);
assert.match(sourceApplyHandoffMarkup, /Studio source apply handoff/);
...
assert.match(sourceApplyHandoffMarkup, /edit draft-preview/);
assert.match(sourceApplyHandoffMarkup, /Copyable inert preview command/);
assert.match(sourceApplyHandoffMarkup, /disabled/);
...
assert.equal(a11yNavModel.regions[0].id, 'studio-nav');
...
assert.equal(cockpit.nextStudioFocus(a11yRegions, a11yIds[0], 'next'), a11yIds[1]);
assert.equal(cockpit.nextStudioFocus(a11yRegions, a11yIds[a11yIds.length - 1], 'next'), a11yIds[0]);
assert.equal(cockpit.nextStudioFocus(a11yRegions, a11yIds[0], 'prev'), a11yIds[a11yIds.length - 1]);
assert.equal(cockpit.nextStudioFocus(a11yRegions, a11yIds[1], 'prev'), a11yIds[0]);
assert.equal(cockpit.nextStudioFocus(a11yRegions, 'does-not-exist', 'next'), a11yIds[0]);
assert.equal(cockpit.nextStudioFocus([], 'x', 'next'), null);
// restore focus resolves a known id or falls back to first
assert.equal(cockpit.restoreStudioFocus(a11yRegions, a11yIds[2]), a11yIds[2]);
assert.equal(cockpit.restoreStudioFocus(a11yRegions, 'missing'), a11yIds[0]);
assert.equal(cockpit.restoreStudioFocus([], 'x'), null);
const a11ySurface = cockpit.renderStudioAccessibilityNavSurface(run);
...
assert.match(a11ySurface, /Studio navigation/);
assert.match(a11ySurface, /Contrast and readability smoke/);
// regions render in declared order
assert.ok(a11ySurface.indexOf('Studio navigation') < a11ySurface.indexOf('Live preview panel'), 'regions render in declared order');
...
// surface is wired into the integration render

...
assert.match(roadmapDoc, /3D Capability Gate v1 \(#596-#608\)/);
assert.match(roadmapDoc, /Keep it a scoped capability gate, not\s+a broad 3D parity, production-engine, native-export, plugin-runtime, or Godot\s+replacement claim/);
assert.match(roadmapDoc, /Treat Production 2D Engine Core v1 as complete but bounded/);
assert.match(roadmapDoc, /not authority to\s+claim production readiness, shipped-game maturity, secure sandboxing, broad\s+compatibility stability, native export, plugin runtime, hosted\/cloud behavior,\s+source apply, public launch, or Godot replacement status/);

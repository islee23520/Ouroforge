const OuroforgeCockpit = (() => {
  ...
  function studioKeyboardNavModel(run = null) {
    ...
    const regions = [
      { id: 'studio-nav', label: 'Studio navigation', role: 'navigation', shortcut: 'Alt+1', detail: 'jump between demo surfaces' },
      { id: 'live-preview', label: 'Live preview panel', role: 'region', shortcut: 'Alt+2', detail: 'ephemeral probe controls' },
      { id: 'scene-tree', label: 'Scene tree', role: 'tree', shortcut: 'Alt+3', detail: hasRun ? 'entity tree navigation' : 'scene data not loaded' },
      { id: 'inspector-fields', label: 'Inspector fields', role: 'form', shortcut: 'Alt+4', detail: 'read/edit field navigation' },
      { id: 'command-palette', label: 'Command generation', role: 'region', shortcut: 'Alt+5', detail: 'Rust command preview' },
      { id: 'evidence-pane', label: 'Evidence pane', role: 'region', shortcut: 'Alt+6', detail: 'read-only evidence inspection' },
    ];
    // Documented contrast/readability smoke pairs (intent only; no color computation).
    const contrastPairs = [
      { region: 'studio-nav', label: 'Studio navigation', intent: 'navigation labels readable at default zoom' },
      { region: 'scene-tree', label: 'Scene tree', intent: 'tree node labels readable against panel background' },
      { region: 'inspector-fields', label: 'Inspector fields', intent: 'field labels and values readable for color-blind users' },
    ];
  ...
  function renderStudioAccessibilityNavSurface(run = null) {
    ...
    return `<section class="panel" id="studio-accessibility-nav"><h2>Keyboard navigation and landmarks</h2>
      <nav role="navigation" aria-label="Studio keyboard navigation guide">
        <p class="hint">Read-only keyboard-shortcut and landmark guide. No apply, merge, command execution, or trusted-write controls are exposed.</p>
        <ol role="list" aria-label="Ordered focusable regions">${items}</ol>
      </nav>
      <h3>Contrast and readability smoke</h3>
  ...
  function studioSurfaceSummary(run) {
    ...
    return [
      ...
      { id: 'source-apply-review', label: 'Source apply review', present: Boolean(sourceApplyReviewReadModel(run).present), detail: `${sourceApplyReviewReadModel(run).gates.filter((gate) => gate.present).length}/${sourceApplyReviewReadModel(run).gates.length} review gate(s) present` },
      { id: 'export-inspection', label: 'Export inspection', present: Boolean(exportInspectionReadModel(run).present), detail: exportInspectionReadModel(run).present ? `${exportInspectionReadModel(run).target} · ${exportInspectionReadModel(run).assetCount} asset(s)` : 'no export evidence bundle exported' },
      { id: 'project-overview', label: 'Project overview', present: Boolean(projectOverviewReadModel(run).present), detail: projectOverviewReadModel(run).present ? `${projectOverviewReadModel(run).projectId}` : 'no project overview exported' },
      ...
      { id: 'evaluator-depth-inspection', label: 'Evaluator depth inspection', present: Boolean(evaluatorDepthReadModel(run).present), detail: `${Object.keys(evaluatorDepthReadModel(run).categories || {}).length} gate categor(ies)` },
      { id: 'prototype-planning-inspection', label: 'GDD prototype planning inspection', present: Boolean(prototypePlanningInspectionModel(run).present), detail: `${prototypePlanningInspectionModel(run).panels.length} read-only prototype panel(s)` },
  ...
  const SOURCE_APPLY_REVIEW_GATES = [
    { id: 'apply-transaction', label: 'Apply transaction' },
    { id: 'review-decision', label: 'Review decision enforcement' },
    { id: 'stale-guard', label: 'Stale target guard' },
    { id: 'sandbox-promotion', label: 'Sandbox-to-trusted promotion' },
    { id: 'rollback', label: 'Rollback snapshot / recovery' },
    { id: 'verification', label: 'Verification' },
    { id: 'rerun-comparison', label: 'Rerun / comparison' },
    { id: 'audit-ledger', label: 'Audit ledger' },
    { id: 'evidence-bundle', label: 'Evidence bundle' },
  ];

  const SOURCE_APPLY_FORBIDDEN_ACTIONS = [
    'apply_patch',
    'merge_branch',
    'self_approve',
    'reviewer_bypass',
  ...
  function sourceApplyReviewRefs(refs) {
  ...
  // Read-only aggregation of source-apply review readiness exported by Rust.
  // It does not apply patches, accept reviews, or execute anything.
  function sourceApplyReviewReadModel(run = {}) {
    const model = run?.source_apply_review || run?.sourceApplyReview || null;
    if (!model || typeof model !== 'object') {
      return {
        ...
        gates: [],
        malformedReasons: [],
        forbiddenActions: SOURCE_APPLY_FORBIDDEN_ACTIONS,
        emergencyHold: null,
        boundary:
          'No source apply review read model is attached to dashboard-data.json. Read-only Studio inspection only.',
      };
    }
    const gatesAreArray = Array.isArray(model.gates);
    const gateEntries = gatesAreArray ? model.gates : [];
    const malformedReasons = Array.isArray(model.malformedReasons || model.malformed_reasons)
      ...
      : [];
    if (!gatesAreArray && typeof model.gates !== 'undefined') {
      malformedReasons.push('gates must be an array');
    }
    const byId = new Map();
    for (const entry of gateEntries) {
      if (entry && typeof entry === 'object') {
        byId.set(entry.id || entry.gate || entry.kind, entry);
      }
    }
    const gates = SOURCE_APPLY_REVIEW_GATES.map((gate) => {
      const entry = byId.get(gate.id) || {};
      const present = Boolean(entry.present ?? (entry.status && entry.status !== 'missing'));
      const blockers = Array.isArray(entry.blockers || entry.blockedReasons || entry.blocked_reasons)
        ? (entry.blockers || entry.blockedReasons || entry.blocked_reasons)
        : [];
      return {
        id: gate.id,
        label: gate.label,
        present,
        status: entry.status || (present ? 'present' : 'missing'),
        detail: entry.detail || entry.summary || '',
        blockers,
        refs: sourceApplyReviewRefs(entry.refs || entry.evidenceRefs || entry.evidence_refs),
      };
    });
    const allReady = gates.every((gate) => gate.present && !gate.blockers.length);
    return {
      present: Boolean(model.present ?? gateEntries.length),
      status: model.status || (malformedReasons.length ? 'malformed' : allReady ? 'ready' : 'incomplete'),
      gates,
      malformedReasons,
      forbiddenActions: Array.isArray(model.forbiddenActions || model.forbidden_actions)
        ? (model.forbiddenActions || model.forbidden_actions)
        : SOURCE_APPLY_FORBIDDEN_ACTIONS,
      emergencyHold: model.emergencyHold || model.emergency_hold || null,
      boundary:
        model.boundary ||
        'Read-only Studio source apply review surface; no apply, command execution, merge, self-approval, reviewer bypass, or trusted browser writes.',
    };
  }

  function renderSourceApplyReviewSurface(run) {
    const model = sourceApplyReviewReadModel(run);
    const hint =
      '<p class="hint">Read-only Studio inspection of review-gated source apply readiness from exported JSON only; the browser cannot apply patches, accept reviews, bypass review gates, merge branches, roll back, execute commands, or write trusted files.</p>';
    if (!model.present) {
      return `<section id="source-apply-review" class="panel"><h2>Source apply review</h2><p class="empty">No source apply review read model is exported for this run.</p>${hint}</section>`;
    }
    const gateRows = model.gates
      .map((gate) => {
        const detail = gate.detail ? ` · ${escapeText(gate.detail)}` : '';
        const blocked = gate.blockers.length ? `<br><small>Blocked: ${escapeText(gate.blockers.join(' · '))}</small>` : '';
        const refs = gate.refs.length ? `<br><small>Refs: ${escapeText(gate.refs.join(' · '))}</small>` : '';
        return `<li class="surface-row"><strong>${escapeText(gate.label)}</strong> ${surfaceState(gate.present && !gate.blockers.length, gate.status)}<small>${detail}</small>${blocked}${refs}</li>`;
      })
      .join('');
    const malformed = model.malformedReasons.length
      ? `<h3>Malformed read-model notices</h3><ul>${model.malformedReasons.map((reason) => `<li class="surface-row">${escapeText(reason)}</li>`).join('')}</ul>`
      : '';
    const hold = model.emergencyHold
      ? `<p class="hint">Emergency hold: ${escapeText(typeof model.emergencyHold === 'string' ? model.emergencyHold : model.emergencyHold.reason || model.emergencyHold.status || 'active')}. Apply remains blocked.</p>`
      : '';
    return `<section id="source-apply-review" class="panel"><h2>Source apply review</h2>
      ${hint}
      <p>Overall readiness: ${surfaceState(model.status === 'ready', model.status)}</p>
      ${hold}
      <h3>Review gates</h3><ul>${gateRows}</ul>
      <h3>Forbidden actions (always blocked in the browser)</h3>
      <ul>${model.forbiddenActions.map((action) => `<li class="surface-row">${escapeText(action)}</li>`).join('')}</ul>
      ${malformed}
    </section>`;
  }

  // Read-only normalization of an exported export-evidence bundle (#729 shape).
  // The Studio surface displays it only; it never runs export, publish, deploy,
  // sign, upload, or any command.
  function exportInspectionReadModel(run = {}) {
    const model = run?.export_evidence || run?.exportEvidence || null;
    if (!model || typeof model !== 'object') {
      return {
        present: false,
        boundary:
          'No export evidence bundle is attached to dashboard-data.json. Read-only Studio inspection only.',
      };
    }
    const links = model.links || {};
    const profile = model.profile || {};
    const plan = model.plan || {};
    const manifest = model.assetManifest || model.asset_manifest || {};
    const fingerprint = model.fingerprint || {};
    const verification = model.verification || {};
    const checks = Array.isArray(verification.checks) ? verification.checks : [];
    const assetEntries = Array.isArray(manifest.entries) ? manifest.entries : [];
    const checksums = Array.isArray(fingerprint.artifactChecksums || fingerprint.artifact_checksums)
      ? (fingerprint.artifactChecksums || fingerprint.artifact_checksums)
      : [];
    // Publish/release targets are always blocked; web-local and web-static-bundle
    // are the only allowed local targets.
    const allowedTargets = ['web-local', 'web-static-bundle'];
    const target = profile.exportTarget || profile.export_target || plan.exportTarget || plan.export_target || 'unknown';
    return {
      present: true,
      runId: links.runId || links.run_id || 'unlinked',
      projectId: links.projectId || links.project_id || profile.projectId || profile.project_id || 'unlinked',
      target,
      outputDir: plan.outputDir || plan.output_dir || profile.outputDir || profile.output_dir || 'unknown',
      assetCount: assetEntries.length,
      checksumCount: checksums.length,
      verificationStatus: verification.verdict || 'unknown',
      checkCount: checks.length,
      verdict: model.verdict || 'unknown',
      publishBlocked: !allowedTargets.includes(target)
        ? `target ${target} is not an allowed local target`
        : 'publish/deploy/sign/upload blocked (local export only)',
      evidenceFile: 'export-evidence.json',
      boundary:
        model.boundary ||
        'Read-only Studio export inspection; the browser cannot run export, publish, deploy, sign, upload, or any command.',
    };
  }

  function renderExportInspectionSurface(run) {
    const model = exportInspectionReadModel(run);
    const hint =
      '<p class="hint">Read-only Studio inspection of exported export-evidence JSON only; the browser cannot run export, publish, deploy, sign, upload, or any command.</p>';
    if (!model.present) {
      return `<section id="export-inspection" class="panel"><h2>Export inspection</h2><p class="empty">No export evidence bundle is exported for this run.</p>${hint}</section>`;
    }
    return `<section id="export-inspection" class="panel"><h2>Export inspection</h2>
      ${hint}
      <ul>
        <li class="surface-row"><strong>Target</strong> ${surfaceState(model.target !== 'unknown', escapeText(model.target))}</li>
        <li class="surface-row"><strong>Output path</strong> <small>${escapeText(model.outputDir)}</small></li>
        <li class="surface-row"><strong>Assets</strong> <small>${escapeText(model.assetCount)} packaged asset(s)</small></li>
        <li class="surface-row"><strong>Checksums</strong> <small>${escapeText(model.checksumCount)} artifact checksum(s)</small></li>
        <li class="surface-row"><strong>Verification</strong> ${surfaceState(model.verificationStatus === 'pass', model.verificationStatus)}<small> · ${escapeText(model.checkCount)} check(s)</small></li>
        <li class="surface-row"><strong>Verdict</strong> ${surfaceState(model.verdict === 'pass', model.verdict)}</li>
        <li class="surface-row"><strong>Publish/release</strong> <span class="status-idle">blocked</span><small> · ${escapeText(model.publishBlocked)}</small></li>
        <li class="surface-row"><strong>Evidence bundle</strong> <small>${escapeText(model.evidenceFile)} (run ${escapeText(model.runId)}, project ${escapeText(model.projectId)})</small></li>
      </ul>
    </section>`;
  }

  // Read-only project overview aggregation (#758). Side-effect free: it reads
  // exported JSON only and never mutates trusted files or runs commands.
  function projectOverviewReadModel(run = {}) {
    const model = run?.project_overview || run?.projectOverview || null;
    if (!model || typeof model !== 'object') {
      return {
        present: false,
        diagnostics: ['No project overview read model is attached to dashboard-data.json.'],
        sections: [],
        boundary: 'Read-only Studio project overview; no trusted writes or command execution.',
      };
    }
    ...
    const num = (...keys) => {
      for (const key of keys) {
        const value = counts[key] ?? model[key];
        if (typeof value === 'number') return value;
      }
      return null;
    };
    const sections = [
      { id: 'scenes', label: 'Scenes', count: num('scenes', 'sceneCount', 'scene_count') },
      { id: 'assets', label: 'Assets', count: num('assets', 'assetCount', 'asset_count') },
      { id: 'scenarios', label: 'Scenarios', count: num('scenarios', 'scenarioCount', 'scenario_count') },
      { id: 'evidence-runs', label: 'Evidence runs', count: num('evidenceRuns', 'evidence_runs', 'runCount', 'run_count') },
    ];
    const rawExportStatus = model.exportStatus ?? model.export_status;
    const rawPluginStatus = model.pluginRegistryStatus ?? model.plugin_registry_status;
    const exportStatus = typeof rawExportStatus === 'string' && rawExportStatus ? rawExportStatus : 'unknown';
    const pluginStatus = typeof rawPluginStatus === 'string' && rawPluginStatus ? rawPluginStatus : 'unknown';
    const validationSummary = Array.isArray(model.validationSummary || model.validation_summary)
      ? (model.validationSummary || model.validation_summary)
      : [];
    const diagnostics = Array.isArray(model.diagnostics) ? [...model.diagnostics] : [];
    for (const section of sections) {
      if (section.count === null) {
        diagnostics.push(`${section.label} count is missing or invalid`);
      }
    }
    // A run can omit export or plugin registry sub-state; surface an actionable
    // diagnostic for each unhealthy status instead of silently rendering `unknown`.
    const UNHEALTHY_STATUSES = ['unknown', 'missing', 'invalid', 'error', 'failed', 'unavailable'];
    const statusDiagnostic = (label, status, action) => {
      const normalized = String(status).toLowerCase();
      if (!UNHEALTHY_STATUSES.includes(normalized)) {
        return null;
      }
      const cause = normalized === 'unknown' ? 'missing or not exported' : normalized;
      return `${label} is ${cause}; ${action}`;
    };
    const exportDiagnostic = statusDiagnostic('Export status', exportStatus, 'export the build/export summary to populate it.');
    if (exportDiagnostic) {
      diagnostics.push(exportDiagnostic);
    }
    const pluginDiagnostic = statusDiagnostic('Plugin registry status', pluginStatus, 'run plugin discovery to populate the registry status.');
    if (pluginDiagnostic) {
      diagnostics.push(pluginDiagnostic);
    }
    return {
      present: Boolean(model.present ?? (project.id || project.projectId || sections.some((section) => section.count !== null))),
      projectId: project.id || project.projectId || project.project_id || 'unlinked',
      title: project.title || 'untitled project',
      version: project.version || 'unversioned',
      sections,
      exportStatus,
      pluginStatus,
      validationSummary,
      diagnostics,
      boundary:
        model.boundary || 'Read-only Studio project overview; no trusted writes or command execution.',
    };
  }

  function renderProjectOverviewSurface(run) {
    const model = projectOverviewReadModel(run);
    const hint =
      '<p class="hint">Read-only Studio project overview aggregated from exported JSON only; no trusted writes, mutation controls, command bridge, or hidden execution.</p>';
    if (!model.present) {
      const diagnostics = model.diagnostics.map((reason) => `<li class="surface-row">${escapeText(reason)}</li>`).join('');
      return `<section id="project-overview" class="panel"><h2>Project overview</h2><p class="empty">No project overview is exported for this run.</p><ul>${diagnostics}</ul>${hint}</section>`;
    }
    const sectionRows = model.sections
      .map((section) => `<li class="surface-row"><strong>${escapeText(section.label)}</strong> ${surfaceState(section.count !== null, section.count === null ? 'missing' : `${section.count}`)}</li>`)
      .join('');
    const validationRows = model.validationSummary.length
      ? `<ul>${model.validationSummary.slice(0, 12).map((item) => `<li class="surface-row">${escapeText(item)}</li>`).join('')}</ul>`
      : '<p class="empty compact">No validation summary exported.</p>';
    const diagnosticsBlock = model.diagnostics.length
      ? `<h3>Diagnostics</h3><ul>${model.diagnostics.map((reason) => `<li class="surface-row">${escapeText(reason)}</li>`).join('')}</ul>`
      : '';
    return `<section id="project-overview" class="panel"><h2>Project overview</h2>
      ${hint}
      <p><strong>${escapeText(model.title)}</strong> · ${escapeText(model.projectId)} · v${escapeText(model.version)}</p>
      <h3>Counts</h3><ul>${sectionRows}</ul>
      <h3>Export status</h3><p>${surfaceState(model.exportStatus !== 'unknown', model.exportStatus)}</p>
      <h3>Plugin registry status</h3><p>${surfaceState(model.pluginStatus !== 'unknown', model.pluginStatus)}</p>
      <h3>Validation summary</h3>${validationRows}
      ${diagnosticsBlock}
    </section>`;
  }
  ...
  function studioSourceApplyHandoffModel(draft, run) {
    ...
    const guard = sourcePatchStaleTargetGuardArtifacts(run)[0]?.value || {};
    const transaction = sourcePatchApplyTransactionArtifacts(run)[0]?.value || {};
    ...
    const draftTarget = (draft && typeof draft === 'object' && draft.target && typeof draft.target === 'object') ? draft.target : {};
    const exportedTarget = (exported && typeof exported.target === 'object' && exported.target) ? exported.target : {};

    const path = exportedTarget.path || draftTarget.path || txTarget.path || guardTarget.path || null;
    const expectedHash = exportedTarget.expected_hash || exportedTarget.expectedHash
      || (draft && (draft.beforeHash || draft.before_hash))
      || guardTarget.expectedHash || guardTarget.expected_hash || null;
    const staleRaw = exportedTarget.stale;
    const guardStatus = guard.readModel?.status || guard.read_model?.status || guard.status || '';
    const inferredStale = typeof staleRaw === 'boolean'
      ? staleRaw
    ...
    const draftDiffSummary = exported?.draft_diff_summary || exported?.draftDiffSummary
      || (draft && (draft.expectedAfterSummary || draft.expected_after_summary))
      || (operations.length ? `${operations.length} proposed operation(s) pending trusted source apply review` : 'No draft diff summary recorded.');

    const verificationHints = Array.isArray(exported?.verification_hints || exported?.verificationHints)
      ? (exported.verification_hints || exported.verificationHints)
      : [
        'Copy the inert preview command and run it via the trusted Rust CLI outside the browser.',
        'Apply is handled only by the Safe Source Apply review gates, never by Studio.',
        'A non-self accepted review decision plus rollback and evidence refs are required before any trusted apply.',
      ];

    const present = Boolean(exported ? (exported.present ?? true) : (path || draft));
    return {
      present,
      empty_state: exported?.empty_state || exported?.emptyState || 'No Studio source apply handoff inputs are available; author a draft or load source apply preview evidence first.',
      target: {
        path: path || 'unknown target',
        expected_hash: expectedHash || 'unknown',
        stale: inferredStale,
      },
      draft_diff_summary: draftDiffSummary,
  ...
  function renderStudioSourceApplyHandoffSurface(run) {
    const state = studioDraftAuthoringState(run);
    const draft = state.drafts[0] || null;
    const exported = (run && (run.source_apply_handoff || run.sourceApplyHandoff)) || null;
    const model = studioSourceApplyHandoffModel(draft, run);
    if (!model.present && !exported && !draft) {
      return `<section id="studio-source-apply-handoff" class="panel"><h2>Studio source apply handoff</h2><p class="empty">${escapeText(model.empty_state)}</p><p class="hint">${escapeText(model.apply_boundary)}</p></section>`;
    }
    const previewCommand = studioDraftPreviewCommand(draft, run);
    const hints = model.verification_hints.map((hint) => `<li>${escapeText(hint)}</li>`).join('') || '<li>No verification hints recorded.</li>';
    ...
    return `<section id="studio-source-apply-handoff" class="panel"><h2>Studio source apply handoff</h2>
      ...
      <div class="surface-row">
        ...
        <small>Draft diff: ${escapeText(model.draft_diff_summary)}</small><br>
        <small>Review required: ${escapeText(String(model.review_required))} · Rollback required: ${escapeText(String(model.rollback_required))} · Evidence required: ${escapeText(String(model.evidence_required))}</small><br>
        <small>Forbidden: ${escapeText(model.forbidden_actions.join(' · '))}</small>
      </div>
      <h4>Verification hints</h4><ul>${hints}</ul>
      <h4>Copyable inert preview command</h4><code>${escapeText(previewCommand)}</code>
      <p class="hint">This command is inert text only; Studio does not execute it, apply patches, merge branches, self-approve reviews, or write trusted files.</p>
    </section>`;
  }
  ...
  function tilemapPainterModel(source) {
    const map = source?.tilemap || source?.map || source || {};
    const palette = Array.isArray(map.palette) ? map.palette.map((entry, index) => ({
      tileId: entry?.tileId || entry?.tile_id || `tile-${index + 1}`,
      label: entry?.label || entry?.tileId || entry?.tile_id || `Tile ${index + 1}`,
      solid: entry?.solid === true,
    })) : [];
    const layers = Array.isArray(map.layers) ? map.layers.map((layer, index) => ({
      layerId: layer?.layerId || layer?.layer_id || `layer-${index + 1}`,
      kind: layer?.kind || 'ground',
      cells: Array.isArray(layer?.cells) ? layer.cells : [],
    })) : [];
    const width = Number(map.width || (layers[0]?.cells?.[0]?.length) || 0);
    const height = Number(map.height || layers[0]?.cells?.length || 0);
    return {
      present: source?.present !== false && width > 0 && height > 0,
      mapId: map.mapId || map.map_id || 'unknown-map',
      sourcePath: map.sourcePath || map.source_path || 'examples/tilemap-authoring-v1/maps/<map-id>.tilemap.json',
      baseDigest: source?.baseDigest || source?.base_digest || map.baseDigest || map.base_digest || 'fnv64:uncomputed',
      width,
      height,
      palette,
      layers,
      selectedLayerId: source?.selectedLayerId || source?.selected_layer_id || layers[0]?.layerId || '',
      selectedTileId: source?.selectedTileId || source?.selected_tile_id || palette[0]?.tileId || '',
      boundary: source?.boundary || 'Visual tile painting creates inert draft JSON only; browser cannot write trusted tilemap source, execute commands, or apply reviews.',
    };
  }

  function tilemapPaintDraftOperation(model, x, y, tileId = model.selectedTileId, layerId = model.selectedLayerId) {
    const reasons = [];
    if (!model || !model.present) reasons.push('Tilemap painter model is not present.');
    if (!model.layers.some((layer) => layer.layerId === layerId)) reasons.push(`Unknown layer: ${layerId}`);
    if (tileId !== '.' && !model.palette.some((entry) => entry.tileId === tileId)) reasons.push(`Unknown tile: ${tileId}`);
    if (!Number.isInteger(x) || !Number.isInteger(y) || x < 0 || y < 0 || x >= model.width || y >= model.height) reasons.push('Cell is out of bounds.');
    if (reasons.length) return { validationStatus: 'blocked', blockedReasons: reasons };
    const op = tileId === '.' ? 'erase' : 'paint';
    const operation = op === 'erase'
      ? { op, operationId: `erase-${layerId}-${x}-${y}`, layerId, x, y }
      : { op, operationId: `paint-${layerId}-${x}-${y}`, layerId, x, y, tileId };
    return {
      schemaVersion: 'ouroforge.tilemap-draft.v1',
      draftId: `draft-${model.mapId}-${layerId}-${x}-${y}`,
      target: { path: model.sourcePath, baseDigest: model.baseDigest },
      proposedOperations: [operation],
      validationStatus: 'drafted',
      previewSummary: {
        changedCells: [{ layerId, x, y, before: 'preview-read-model', after: tileId }],
        affectedCollisionCells: model.layers.find((layer) => layer.layerId === layerId)?.kind === 'collision' ? [{ x, y }] : [],
        affectedTriggerMarkers: [],
        generatedPreviewOnly: true,
      },
      guardrails: ['generated preview only; not source', 'review-gated apply remains separate'],
    };
  }

  function renderTilemapPainterSurface(source) {
    const model = tilemapPainterModel(source);
    if (!model.present) {
      return `<section id="tilemap-painter" class="panel"><h2>Tile painter</h2><p class="empty">No tilemap source read model is available.</p><p class="hint">${escapeText(model.boundary)}</p></section>`;
    }
    const palette = model.palette.map((entry) => `<button type="button" data-tile-id="${escapeText(entry.tileId)}" disabled>${escapeText(entry.label)}${entry.solid ? ' · solid' : ''}</button>`).join('');
    const layerOptions = model.layers.map((layer) => `<option value="${escapeText(layer.layerId)}"${layer.layerId === model.selectedLayerId ? ' selected' : ''}>${escapeText(layer.layerId)} · ${escapeText(layer.kind)}</option>`).join('');
    const activeLayer = model.layers.find((layer) => layer.layerId === model.selectedLayerId) || model.layers[0];
    const rows = Array.from({ length: model.height }, (_, y) => {
      const cells = Array.from({ length: model.width }, (_, x) => {
        const tile = activeLayer?.cells?.[y]?.[x] ?? '.';
        return `<button type="button" class="tile-cell" data-x="${x}" data-y="${y}" disabled>${escapeText(tile)}</button>`;
      }).join('');
      return `<div class="tile-row">${cells}</div>`;
    }).join('');
    const exampleDraft = tilemapPaintDraftOperation(model, 0, 0);
    return `<section id="tilemap-painter" class="panel"><h2>Tile painter</h2>
      <p class="hint">${escapeText(model.boundary)} Palette/grid controls are disabled in static evidence; live Studio uses the same draft JSON shape and still cannot apply source writes.</p>
      <div class="field-grid"><div><strong>Map</strong><br>${escapeText(model.mapId)}</div><div><strong>Size</strong><br>${escapeText(model.width)}×${escapeText(model.height)}</div><div><strong>Base digest</strong><br>${escapeText(model.baseDigest)}</div></div>
      <label>Layer <select disabled>${layerOptions}</select></label>
      <div class="tile-palette">${palette}</div>
      <div class="tile-grid" role="grid" aria-label="Tilemap draft grid">${rows}</div>
      <h4>Draft JSON preview</h4><pre>${escapeText(JSON.stringify(exampleDraft, null, 2))}</pre>
      <p class="hint">Generated preview is not source; review-gated apply is outside the browser.</p>
    </section>`;
  }
  ...
  const STUDIO_WORKSPACE_UX_CONTRACT_VERSION = 'studio-workspace-ux-contract-v1';
  const STUDIO_WORKSPACE_UX_SURFACES = [
    {
      id: 'project-open',
      label: 'Project open / create entry',
      hostFile: 'examples/authoring-cockpit/index.html',
      hostSurface: 'renderProjectWorkspaceSurface',
      selector: '#project-workspace',
      stateBoundary: 'source manifest is selected by user/CLI; browser displays metadata and copyable validation commands only',
      writeAuthority: 'none-browser-read-only',
      feedsIssues: ['#2363'],
    },
    {
      id: 'scene-list',
      label: 'Scene list',
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderProjectWorkspaceSurface + renderStudioSceneTreeInspectorSurface',
      selector: '#project-workspace, .scene-tree-inspector',
      stateBoundary: 'scene paths come from project/dashboard read models and remain source-labelled display data',
      writeAuthority: 'draft-only-via-scene-draft-model',
      feedsIssues: ['#2364', '#2365', '#2366'],
    },
    {
      id: 'asset-list',
      label: 'Asset list',
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderStudioAssetBrowserSurface',
      selector: '#studio-asset-browser',
      stateBoundary: 'asset descriptors and generated outputs are escaped read-only evidence',
      writeAuthority: 'none-browser-read-only',
      feedsIssues: ['#2364'],
    },
    {
      ...
      label: 'Run action',
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderProjectRunSurface + renderRunCommandContext',
      selector: '#project-run .command-list, .command-context',
      stateBoundary: 'copyable safe command text only; no browser command bridge or hidden execution',
      writeAuthority: 'trusted-cli-outside-browser',
      feedsIssues: ['#2363'],
    },
    {
      id: 'evidence-browser',
      ...
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderEvidenceBrowser + renderEvidenceTimelineSurface',
      selector: '#run-browser, #evidence-timeline',
      stateBoundary: 'runs/live-observability manifests and dashboard exports are read-only generated state',
      writeAuthority: 'none-browser-read-only',
      feedsIssues: ['#2363', '#2364'],
    },
    {
      id: 'state-labels',
      label: 'Source/generated state labels',
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderStudioSourceGeneratedBrowserSurface',
      selector: '#source-generated-browser',
      stateBoundary: 'hardened path classification labels source, generated, temp, blocked, and unsupported roots',
      writeAuthority: 'none-browser-read-only',
      feedsIssues: ['#2364'],
    },
    {
      id: 'scene-editor',
      label: 'Scene editor draft/review flow',
      hostFile: 'examples/authoring-cockpit/cockpit.js',
      hostSurface: 'renderStudioSceneCanvasSurface + renderStudioDraftAuthoringSurface + renderStudioSourceApplyHandoffSurface',
      selector: '#studio-scene-canvas, #studio-draft-authoring, #studio-source-apply-handoff',
      stateBoundary: 'base-relative draft operations carry base scene digest and require Safe Source Apply handoff',
      writeAuthority: 'draft-only-no-direct-browser-writes',
      feedsIssues: ['#2365', '#2366', '#2367', '#2368'],
    },
  ];

  function studioWorkspaceUxContract() {
    return {
      schemaVersion: STUDIO_WORKSPACE_UX_CONTRACT_VERSION,
      issue: '#2362',
      classification: 'contract-complete',
      hostRoot: 'examples/authoring-cockpit/',
      browserCommandBridge: false,
      sourceOfTruth: 'existing Studio/cockpit JS surfaces; Rust CLI remains trusted authority outside the browser',
      localFirstPersistence: [
        'browser surfaces render escaped read-only exported JSON and in-memory draft previews',
        'generated runs/screenshots/dashboard/live-observability bundles stay under generated roots',
        'trusted source changes require review-gated Safe Source Apply outside the browser',
      ],
      surfaces: STUDIO_WORKSPACE_UX_SURFACES.map((surface) => ({ ...surface })),
    };
  }

  function renderWorkspaceUxContractSurface() {
    const contract = studioWorkspaceUxContract();
    const rows = contract.surfaces.map((surface) => `<li class="surface-row"><strong>${escapeText(surface.label)}</strong> <small>${escapeText(surface.id)}</small><br><small>Host: ${escapeText(surface.hostFile)} :: ${escapeText(surface.hostSurface)}</small><br><small>Selector: ${escapeText(surface.selector)} · Authority: ${escapeText(surface.writeAuthority)}</small><br><small>Boundary: ${escapeText(surface.stateBoundary)}</small><br><small>Feeds: ${escapeText(surface.feedsIssues.join(', '))}</small></li>`).join('');
    return `<section id="workspace-ux-contract" class="panel"><h2>Workspace UX contract</h2>
      <p class="hint">${escapeText(contract.schemaVersion)} maps each M121/M122 workspace element to an existing Studio/cockpit host before run/evidence or editor issues choose implementation surfaces. Browser command bridge: ${escapeText(String(contract.browserCommandBridge))}.</p>
      <ul>${rows}</ul>
      <p class="hint">Local-first persistence: ${escapeText(contract.localFirstPersistence.join(' · '))}</p>
    </section>`;
  }
  ...
  const STUDIO_SOURCE_GENERATED_ROOTS = [
    { kind: 'source', label: 'trusted source', roots: ['examples/playable-demo-v2/', 'examples/game-runtime/', 'examples/authoring-cockpit/', 'seeds/'] },
    { kind: 'generated', label: 'generated evidence', roots: ['runs/', 'examples/evidence-dashboard/'] },
    { kind: 'temp', label: 'temporary worktree/profile', roots: ['tmp/', '.tmp/', 'browser-profiles/'] },
  ];

  function studioPathHardenedViolation(pathValue) {
    const path = String(pathValue == null ? '' : pathValue);
    if (!path) return 'empty path';
    if (path.includes('\\')) return 'backslash path separator is refused by hardened Studio path validation';
    if (path.startsWith('/') || /^[A-Za-z]:[\/]/.test(path)) return 'absolute paths are refused';
    if (/(^|\/)\.\.($|\/)/.test(path)) return 'parent traversal is refused';
    if (/\0|[;&|`$<>]/.test(path)) return 'shell/control characters are refused';
    return '';
  }

  function classifyStudioSourceGeneratedPath(pathValue) {
    const path = String(pathValue == null ? '' : pathValue);
    const violation = studioPathHardenedViolation(path);
    if (violation) {
      return { path, kind: 'blocked', label: 'blocked path', editable: false, generated: false, blockedReasons: [violation] };
    }
    const match = STUDIO_SOURCE_GENERATED_ROOTS.find((entry) => entry.roots.some((root) => path === root.slice(0, -1) || path.startsWith(root)));
    if (!match) {
      return { path, kind: 'unsupported', label: 'unsupported root', editable: false, generated: false, blockedReasons: ['path is outside allowlisted Studio source/generated roots'] };
    }
    return {
      path,
      kind: match.kind,
      label: match.label,
      editable: match.kind === 'source',
      generated: match.kind !== 'source',
      blockedReasons: match.kind === 'source' ? [] : [`${match.label} is read-only in Studio`],
    };
  }

  function studioSourceGeneratedBrowserModel(run = {}) {
    const source = run?.source_generated_browser || run?.sourceGeneratedBrowser || null;
    const rawItems = Array.isArray(source?.items) ? source.items
      : Array.isArray(source?.paths) ? source.paths.map((path) => ({ path }))
      : [];
    const defaultItems = rawItems.length ? rawItems : [
      { id: 'collect-project', path: 'examples/playable-demo-v2/collect-and-exit/ouroforge.project.json' },
      { id: 'game-scene', path: 'examples/game-runtime/scene.json' },
    ...
    const items = defaultItems.map((item, index) => {
      ...
      return {
        ...
        ...classification,
        reason: item?.reason || classification.blockedReasons.join('; '),
      };
    });
    return {
  ...
  function renderStudioSourceGeneratedBrowserSurface(run) {
    const model = studioSourceGeneratedBrowserModel(run);
    const rows = model.items.map((item) => `<li class="surface-row"><strong>${escapeText(item.id)}</strong> ${surfaceState(item.kind === 'source', item.label)}<br><small>${escapeText(item.path)} · editable:${escapeText(String(item.editable))} · generated:${escapeText(String(item.generated))}</small>${item.blockedReasons.length ? `<br><small class="warn">${escapeText(item.blockedReasons.join(' · '))}</small>` : ''}</li>`).join('');
    return `<section id="source-generated-browser" class="panel"><h2>Source / generated browser</h2>
      <p class="hint">${escapeText(model.boundary)}</p>
      <p>Status: ${surfaceState(model.status === 'ready', model.status)} · Scenario Coverage: ${escapeText(model.scenarioCoverage.id)} ${escapeText(model.scenarioCoverage.status)}</p>
      <ul>${rows}</ul>
    </section>`;
  }
  ...
  function liveObservabilityManifestEvidenceModel(run = {}) {
    ...
    const rawManifests = Array.isArray(source) ? source
      : Array.isArray(source?.manifests) ? source.manifests
      : Array.isArray(source?.records) ? source.records
      : [];
    const manifests = rawManifests.map((manifest, index) => {
      const manifestPath = String(manifest?.manifestPath || manifest?.manifest_path || manifest?.path || '');
      const entriesRaw = Array.isArray(manifest?.evidence) ? manifest.evidence
      ...
      const entries = entriesRaw.map((entry, entryIndex) => {
        ...
        const entryBlocked = [];
        if (!path) entryBlocked.push('missing evidence path');
        if (path && (!path.startsWith('runs/live-observability/') || path.includes('..') || path.includes('\\'))) {
          entryBlocked.push('evidence path is outside hardened live-observability generated root');
        }
        return {
          id: entry?.id || entry?.name || `evidence-${entryIndex + 1}`,
          kind: entry?.kind || entry?.type || 'unknown',
          path,
          readOnly: true,
          blockedReasons: entryBlocked,
        };
      });
      return {
        id: manifest?.id || manifest?.runId || manifest?.run_id || `live-observability-${index + 1}`,
        manifestPath: manifestPath || 'missing-manifest.json',
        runId: manifest?.runId || manifest?.run_id || manifest?.id || 'unknown-run',
        entries,
        readOnly: true,
        blockedReasons,
      };
    ...
    return {
      present: Boolean((source && source.present !== false) || manifests.length),
      status: manifests.some((manifest) => manifest.blockedReasons.length || manifest.entries.some((entry) => entry.blockedReasons.length)) ? 'blocked' : (manifests.length ? 'ready' : 'missing'),
      boundary: source?.boundary || 'Read-only live-observability evidence listing. Studio reads manifest.json records only; it does not enumerate arbitrary paths, execute runs, or open a browser command bridge.',
      root: 'runs/live-observability/',
      manifests,
      forbiddenActions: ['browser_command_bridge', 'hidden_command_execution', 'trusted_browser_write', 'auto_run'],
    };
  }

  function renderLiveObservabilityEvidenceSurface(run) {
    const model = liveObservabilityManifestEvidenceModel(run);
    if (!model.present) {
      return `<section id="live-observability-evidence" class="panel"><h2>Live observability evidence</h2><p class="empty">No live-observability manifest records are attached to the loaded run.</p><p class="hint">${escapeText(model.boundary)}</p></section>`;
    }
    const manifestRows = model.manifests.map((manifest) => {
      const entryRows = manifest.entries.length
        ? manifest.entries.map((entry) => `<li>${surfaceState(entry.blockedReasons.length === 0, entry.kind)} <strong>${escapeText(entry.id)}</strong><br><small>${escapeText(entry.path || 'missing path')}</small>${entry.blockedReasons.length ? `<br><small class="warn">${escapeText(entry.blockedReasons.join(' · '))}</small>` : ''}</li>`).join('')
        : '<li>No evidence entries recorded in this manifest.</li>';
      const blocked = manifest.blockedReasons.length ? `<p class="warn">Blocked: ${escapeText(manifest.blockedReasons.join(' · '))}</p>` : '<p class="hint">Manifest path is inside the hardened generated root.</p>';
      return `<article class="surface-row"><strong>${escapeText(manifest.id)}</strong> ${surfaceState(manifest.blockedReasons.length === 0, manifest.manifestPath)}<br><small>Run: ${escapeText(manifest.runId)} · read-only: ${escapeText(String(manifest.readOnly))}</small>${blocked}<ul>${entryRows}</ul></article>`;
    }).join('');
    return `<section id="live-observability-evidence" class="panel"><h2>Live observability evidence</h2>
      <p class="hint">${escapeText(model.boundary)} Root: ${escapeText(model.root)}. Forbidden: ${escapeText(model.forbiddenActions.join(' · '))}.</p>
      <p>Status: ${surfaceState(model.status === 'ready', model.status)}</p>
      ${manifestRows}
    </section>`;
  }
  ...
  function renderProvenanceAuditPanel(run) {
    if (!run) {
      return '<section id="provenance-audit" class="panel"><h2>Provenance audit</h2><p class="empty">No dashboard-data.json run is loaded yet.</p></section>';
    }
    ...
    const rows = changes.map((change, index) => {
      ...
      return `<article class="surface-row provenance-change" data-change-id="${escapeText(id)}"><strong>${escapeText(id)}</strong> ${surfaceState(true, status)} <small>${escapeText(change.summary || change.title || 'trusted change record')}</small>
        <h3>Full intent-to-promotion chain</h3><ol>${chainRows}</ol>
        <h3>Replay status</h3><div class="field-grid">${replayFields}</div>
        <h3>Human decision record</h3>
        <p class="hint">Decision ${escapeText(decision)} · reviewer ${escapeText(reviewer)}${signoff.recorded_at || signoff.recordedAt ? ` · ${escapeText(signoff.recorded_at || signoff.recordedAt)}` : ''}</p>
        ${rationale ? `<p class="hint">Rationale: ${escapeText(rationale)}</p>` : ''}
      </article>`;
    }).join('');
    return `<section id="provenance-audit" class="panel"><h2>Provenance audit</h2>
      <p class="hint">Read-only provenance bundle from exported JSON. Sign-off is presented only as a human decision record; this panel has no write, promote, approve, mutation, command, or auto-approval affordance.</p>
      <div class="field-grid">
        <div><strong>Bundle</strong><br>${escapeText(source.bundle_id || source.bundleId || 'unknown')}</div>
        <div><strong>Status</strong><br>${escapeText(source.status || 'recorded')}</div>
        <div><strong>Trusted changes</strong><br>${escapeText(changes.length)}</div>
      </div>
      ${refs.length ? `<h3>Provenance refs</h3>${renderRefLinks(refs, run)}` : ''}
      ${rows || '<p class="empty compact">No trusted changes are recorded in the provenance bundle.</p>'}
    </section>`;
  }
  ...
  function studioCommandRegistry() {
    return [
      ...
      { id: 'reset-layout', label: 'Reset layout', kind: 'navigation', description: 'Restore the default cockpit panel layout (view only).' },
    ];
  }
  ...
  function renderStudioCommandPaletteSurface(run) {
    ...
    const options = commands.map((command) => `<li class="surface-row" role="option" aria-label="${escapeText(command.label)}" data-command-id="${escapeText(command.id)}"><strong>${escapeText(command.label)}</strong> <span class="hint">(${escapeText(command.kind)})</span><br><small>${escapeText(command.description)}</small></li>`).join('');
  ...
  function loopCoverageInspectionModel(run = {}) {
    ...
    if (!source || typeof source !== 'object') {
      return {
        ...
        verdict: { state: 'insufficient-data', reasons: ['No loop coverage evidence is attached to this run.'] },
        counts: { loopProduced: 0, loopVerified: 0, manual: 0, totalTrusted: 0 },
        fractions: { loopProduced: 0, loopVerified: 0, manual: 0, loopCovered: null },
        boundary: 'Loop coverage inspection is read-only and cannot infer missing attribution.',
      };
    }
    ...
    const fractions = source.fractions && typeof source.fractions === 'object' ? source.fractions : {};
    return {
      present: true,
      schemaVersion: source.schemaVersion || source.schema_version || 'loop-coverage-metric-v1',
      ...
      verdict: {
        state: verdict.state || 'insufficient-data',
        baselineLoopCovered: verdict.baselineLoopCovered ?? verdict.baseline_loop_covered ?? null,
        currentLoopCovered: verdict.currentLoopCovered ?? verdict.current_loop_covered ?? null,
      ...
      fractions: {
        ...
        loopCovered: fractions.loopCovered ?? fractions.loop_covered ?? null,
      },
      boundary: source.boundary || 'Loop coverage is descriptive authorship attribution only and Studio inspection is read-only.',
    };
  }

  function renderLoopCoverageInspectionSurface(run) {
    const model = loopCoverageInspectionModel(run);
    const percent = (value) => Number.isFinite(Number(value)) ? `${(Number(value) * 100).toFixed(1)}%` : 'n/a';
    if (!model.present) {
      return `<section id="loop-coverage-inspection" class="panel"><h2>Loop coverage</h2><p class="empty">${escapeText(model.verdict.reasons[0])}</p><p class="hint">${escapeText(model.boundary)} Studio does not compute trusted attribution, write files, execute commands, or promote regressions.</p></section>`;
    }
    const reasons = model.verdict.reasons.length
      ? `<ul>${model.verdict.reasons.map((reason) => `<li>${escapeText(reason)}</li>`).join('')}</ul>`
      : '<p class="hint">No verdict reasons reported.</p>';
    const regressed = model.verdict.state === 'regressed';
    return `<section id="loop-coverage-inspection" class="panel"><h2>Loop coverage</h2>
      <p class="hint">${escapeText(model.boundary)} Studio renders this evidence read-only; it cannot compute trusted attribution, write files, execute commands, or promote regressions.</p>
      <div class="field-grid">
        <div><strong>Verdict</strong><br>${surfaceState(!regressed, model.verdict.state)}</div>
        <div><strong>Loop covered</strong><br>${escapeText(percent(model.fractions.loopCovered))}</div>
        <div><strong>Baseline</strong><br>${escapeText(percent(model.verdict.baselineLoopCovered))}</div>
        <div><strong>Threshold</strong><br>${escapeText(percent(model.verdict.dropThreshold))}</div>
      </div>
      <div class="field-grid">
        <div><strong>Loop-produced</strong><br>${escapeText(model.counts.loopProduced)} · ${escapeText(percent(model.fractions.loopProduced))}</div>
        <div><strong>Loop-verified</strong><br>${escapeText(model.counts.loopVerified)} · ${escapeText(percent(model.fractions.loopVerified))}</div>
        <div><strong>Manual</strong><br>${escapeText(model.counts.manual)} · ${escapeText(percent(model.fractions.manual))}</div>
        <div><strong>Total trusted</strong><br>${escapeText(model.counts.totalTrusted)}</div>
      </div>
      <div class="hint">Schema: ${escapeText(model.schemaVersion)} · project ${escapeText(model.projectId || 'unknown')} · run ${escapeText(model.runId || 'unknown')} · baseline ${escapeText(model.baselineRef || 'none')}</div>
      <h3>Verdict reasons</h3>${reasons}
    </section>`;
  }

  function trustGradientInspectionModel(run) {
    const source = run?.trust_gradient || run?.trustGradient || null;
    const boundary = 'Read-only inspection of exported trust-gradient evidence; Studio does not enable autonomy, toggle the kill switch, apply, merge, or execute commands. Bounded auto-apply decisions and the kill switch are owned by trusted Rust/local paths. The kill switch is not browser-actuated.';
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        present: false,
        empty_state: source?.empty_state || source?.emptyState || 'No trust-gradient auto-apply evidence is available for this run.',
        boundary,
    ...
    const entries = Array.isArray(source.entries) ? source.entries.map((entry, index) => {
      ...
      return {
        ...
        proposalRef: entry?.proposal_ref || entry?.proposalRef || `entry-${index + 1}`,
        tier: entry?.tier || 'unknown',
        applyResult: entry?.apply_result || entry?.applyResult || 'unknown',
        rollbackTransaction: handle.apply_transaction_id || handle.applyTransactionId || 'none',
      };
    }) : [];
    return {
      present: true,
      empty_state: source.empty_state || source.emptyState || 'No trust-gradient auto-apply evidence is available for this run.',
      boundary: source.boundary || boundary,
      autonomyEnabled: source.autonomy_enabled === true || source.autonomyEnabled === true,
      killEngaged: killSwitch.engaged === true,
      killReason: killSwitch.reason || (killSwitch.engaged === true ? 'unspecified' : 'none'),
      budgetRemaining: budget.remaining === undefined || budget.remaining === null ? 'unknown' : String(budget.remaining),
      budgetCost: budget.cost === undefined || budget.cost === null ? 'unknown' : String(budget.cost),
      entries,
    };
  }

  function renderTrustGradientInspectionSurface(run) {
    const model = trustGradientInspectionModel(run);
    if (!model.present) {
      return `<section id="studio-trust-gradient-panel" class="panel"><h2>Trust gradient</h2><p class="empty">${escapeText(model.empty_state)}</p><p class="hint">${escapeText(model.boundary)}</p></section>`;
    }
    const entryRows = model.entries.length
      ? `<ul>${model.entries.map((entry) => {
          const applied = entry.applyResult === 'auto-applied' || entry.applyResult === 'autoApplied';
          return `<li>${surfaceState(applied, escapeText(entry.applyResult))} seq ${escapeText(String(entry.sequence))} · ${escapeText(entry.proposalRef)} · tier ${escapeText(entry.tier)} · rollback ${escapeText(entry.rollbackTransaction)}</li>`;
        }).join('')}</ul>`
      : '<p class="hint">No auto-apply audit entries recorded.</p>';
    return `<section id="studio-trust-gradient-panel" class="panel"><h2>Trust gradient</h2>
      <p class="hint">${escapeText(model.boundary)}</p>
      <p>Autonomy: ${surfaceState(model.autonomyEnabled && !model.killEngaged, model.autonomyEnabled ? 'enabled (opt-in)' : 'disabled (default no auto-apply)')}</p>
      <p>Kill switch: ${surfaceState(!model.killEngaged, model.killEngaged ? 'engaged · autonomy halted' : 'disengaged')} · reason ${escapeText(model.killReason)}</p>
      <p>Risk budget: remaining ${escapeText(model.budgetRemaining)} · cost ${escapeText(model.budgetCost)}</p>
      <h4>Append-only auto-apply audit log (read-only)</h4>${entryRows}
      <p class="hint">Bounded, reversible, audited autonomy is not auto-merge, self-approval, or a quality guarantee. Studio remains read-only.</p>
    </section>`;
  }
  ...
  function evaluatorDepthReadModel(run = {}) {
    const verdict = run?.verdict && typeof run.verdict === 'object' ? run.verdict : {};
    const depth = run?.evaluator_depth || run?.evaluatorDepth || {};
    const categories = verdict.gateCategories || verdict.gate_categories || depth.gateCategories || depth.gate_categories || {};
    const summary = verdict.gateSummary || verdict.gate_summary || depth.gateSummary || depth.gate_summary || [];
    const visual = verdict.visual || verdict.visualGateVerdicts || verdict.visual_gate_verdicts || depth.visual || depth.visualGateVerdicts || depth.visual_gate_verdicts || [];
    const semantic = verdict.semantic || verdict.semanticInvariantVerdicts || verdict.semantic_invariant_verdicts || depth.semantic || depth.semanticInvariantVerdicts || depth.semantic_invariant_verdicts || [];
    return {
      present: Boolean(Object.keys(categories || {}).length || summary.length || visual.length || semantic.length),
      categories: categories || {},
      summary: Array.isArray(summary) ? summary : [],
      visual: Array.isArray(visual) ? visual : [],
      semantic: Array.isArray(semantic) ? semantic : [],
    };
  }

  function renderEvaluatorDepthInspectionSurface(run) {
    const model = evaluatorDepthReadModel(run);
    if (!model.present) {
      return `<section id="evaluator-depth-inspection" class="panel"><h2>Evaluator depth inspection</h2><p class="empty">No evaluator-depth verdict categories are exported for this run.</p><p class="hint">Read-only Studio inspection from exported JSON only; no browser-side trusted writes, mutation controls, command bridge, hidden execution, auto-fix, auto-merge, or quality scoring.</p></section>`;
    }
    const keys = ['mechanical', 'runtime', 'visual', 'semantic'];
    const categoryRows = keys.map((key) => {
      const raw = model.categories[key] || model.categories[key.toUpperCase()] || {};
      const category = typeof raw === 'string' ? { status: raw } : raw;
      const status = category.status || category.verdict || (category.declared === false ? 'neutral' : 'unknown');
      const declared = category.declared === true ? 'declared' : category.declared === false ? 'undeclared / neutral' : 'declaration unknown';
      return `<li class="surface-row"><strong>${escapeText(key)} gate</strong> ${surfaceState(status !== 'missing', status)}<br><small>${escapeText(declared)} · results ${escapeText(category.resultCount ?? category.result_count ?? 0)} · failures ${escapeText(category.failureCount ?? category.failure_count ?? 0)}</small></li>`;
    }).join('');
    const summaryRows = model.summary.length
      ? `<ul>${model.summary.slice(0, 8).map((item) => `<li class="surface-row">${escapeText(item)}</li>`).join('')}</ul>`
      : '<p class="empty compact">No category summary text exported.</p>';
    const visualRows = model.visual.length
      ? model.visual.slice(0, 12).map((gate) => {
          ...
          const threshold = Array.isArray(gate.thresholdSummary || gate.threshold_summary) ? (gate.thresholdSummary || gate.threshold_summary).join(' · ') : (gate.thresholdSummary || gate.threshold_summary || gate.threshold || 'no declared threshold');
          const outcome = gate.outcome || gate.state || gate.status || 'unknown';
          return `<li class="surface-row"><strong>${escapeText(gate.scenarioId || gate.scenario_id || 'scenario')} / ${escapeText(gate.checkpointId || gate.checkpoint_id || 'checkpoint')}</strong> ${surfaceState(outcome !== 'missing', outcome)}<br><small>changed ${escapeText(gate.changedPixels ?? gate.changed_pixels ?? 0)} px · regions ${escapeText(gate.changedRegionCount ?? gate.changed_region_count ?? 0)} · threshold ${escapeText(threshold)}</small>${renderRefLinks(refs, run)}</li>`;
        }).join('')
      : '<li class="surface-row">No visual gate verdicts exported.</li>';
    const semanticRows = model.semantic.length
      ? model.semantic.slice(0, 12).map((gate) => {
          ...
          const state = gate.state || gate.status || gate.outcome || 'unknown';
          return `<li class="surface-row"><strong>${escapeText(gate.invariantId || gate.invariant_id || 'invariant')}</strong> ${surfaceState(state !== 'missing', state)}<br><small>target ${escapeText(gate.targetPath || gate.target_path || 'n/a')} · ${escapeText(gate.reason || gate.message || 'no reason')}</small>${renderRefLinks(refs, run)}</li>`;
        }).join('')
      : '<li class="surface-row">No semantic invariant verdicts exported.</li>';
    return `<section id="evaluator-depth-inspection" class="panel"><h2>Evaluator depth inspection</h2>
      <p class="hint">Read-only Studio inspection from exported JSON only; no browser-side trusted writes, mutation controls, command bridge, hidden execution, auto-fix, auto-merge, or quality scoring.</p>
      <h3>Four-category verdict</h3><ul>${categoryRows}</ul>
      <h3>Gate summary</h3>${summaryRows}
      <h3>Visual diff gate verdicts</h3><ul>${visualRows}</ul>
      <h3>Semantic invariant verdicts</h3><ul>${semanticRows}</ul>
    </section>`;
  }
  ...
  function diagnosticArrayField(value, ...keys) {
    if (!value || typeof value !== 'object') return [];
    for (const key of keys) {
    ...
    safeCheck('blocked-operation', () => {
      ...
      sources.forEach((source) => {
        ...
        list.forEach((item) => {
          ...
          const forbidden = candidates.flatMap((candidate) => diagnosticArrayField(candidate, 'forbiddenActions', 'forbidden_actions'));
          const blocked = candidates.flatMap((candidate) => diagnosticArrayField(candidate, 'blockedReasons', 'blocked_reasons'));
  ...
  function prototypePlanningInspectionModel(run = {}) {
    const model = run?.gdd_prototype_inspection || run?.gddPrototypeInspection || run?.studio_prototype_planning_inspection || run?.studioPrototypePlanningInspection || null;
    if (!model || typeof model !== 'object') {
      return {
        present: false,
        status: 'missing',
        panels: [],
        malformedReasons: ['No GDD-to-prototype planning inspection read model is attached to dashboard-data.json.'],
        boundary: 'Read-only GDD-to-prototype planning inspection; no browser writes, command execution, generation, apply, or merge authority.',
      };
    }
    const rawPanels = model.panels || model.panelSummaries || model.panel_summaries;
    if (!Array.isArray(rawPanels)) {
      return {
        present: Boolean(model.present),
        status: 'malformed',
        panels: [],
        malformedReasons: [
          ...(Array.isArray(model.malformedReasons) ? model.malformedReasons : []),
          'prototype planning panels must be an array',
        ],
        boundary: model.boundary || 'Malformed prototype planning inspection remains read-only.',
      };
    }
    const panels = rawPanels.map((panel) => {
      ...
      const blockers = Array.isArray(panel.blockedReasons || panel.blocked_reasons) ? (panel.blockedReasons || panel.blocked_reasons) : [];
      return {
        panelId: panel.panelId || panel.panel_id || panel.id || 'prototype-panel',
        title: panel.title || panel.label || panel.panelId || panel.panel_id || 'Prototype planning panel',
        status: panel.status || 'unknown',
        artifactKind: panel.artifactKind || panel.artifact_kind || panel.kind || 'prototype-artifact',
        itemCount: Number.isFinite(panel.itemCount) ? panel.itemCount : Number.isFinite(panel.item_count) ? panel.item_count : 0,
        evidenceRefs: refs,
        commands,
        malformedReasons,
        blockedReasons: blockers,
        summary: panel.summary || panel.detail || '',
      };
    });
    return {
      present: Boolean(model.present ?? panels.length),
      ...
      panels,
      malformedReasons: Array.isArray(model.malformedReasons || model.malformed_reasons) ? (model.malformedReasons || model.malformed_reasons) : [],
      boundary: model.boundary || 'Read-only GDD-to-prototype planning inspection; Studio displays escaped artifact status only and cannot write trusted state or execute commands.',
    };
  }

  function renderPrototypePlanningInspectionSurface(run) {
    const model = prototypePlanningInspectionModel(run);
    const hint = '<p class="hint">Escaped read-only GDD-to-prototype inspection: GDD, requirements, mechanics, feasibility, plans, task graph, draft/review/apply status, run evidence, requirement coverage, and journal artifacts remain separated. Copyable commands are inert text only; Studio cannot write files, execute commands, run generation, auto-apply, auto-merge, or perform browser trusted writes.</p>';
    if (!model.present || !model.panels.length) {
      const reasons = model.malformedReasons.length ? `<div class="hint">Malformed/missing: ${escapeText(model.malformedReasons.join(' · '))}</div>` : '';
      return `<section id="prototype-planning-inspection" class="panel"><h2>GDD prototype planning inspection</h2>${hint}<p class="empty">No prototype planning inspection read model is available for this run.</p>${reasons}</section>`;
    }
    const rows = model.panels.map((panel) => {
      const refs = panel.evidenceRefs.length ? panel.evidenceRefs.join(' · ') : 'none';
      const commands = panel.commands.length ? panel.commands.map((command) => typeof command === 'string' ? command : command.command || command.text || '').filter(Boolean).join(' · ') : 'none';
      const malformed = panel.malformedReasons.length ? `<div class="hint">Malformed: ${escapeText(panel.malformedReasons.join(' · '))}</div>` : '';
      const blockers = panel.blockedReasons.length ? `<div class="hint">Blockers: ${escapeText(panel.blockedReasons.join(' · '))}</div>` : '<div class="hint">No blockers reported.</div>';
      return `<li class="surface-row"><strong>${escapeText(panel.title)}</strong> ${surfaceState(panel.status === 'ready' || panel.status === 'pass' || panel.status === 'complete', panel.status)}<br><small>Panel ${escapeText(panel.panelId)} · ${escapeText(panel.artifactKind)} · ${escapeText(panel.itemCount)} item(s)</small><div class="hint">Evidence refs: ${escapeText(refs)}</div><div class="hint">Inert command text: ${escapeText(commands)}</div>${blockers}${malformed}<div class="hint">${escapeText(panel.summary || 'Read-only artifact status row.')}</div></li>`;
    }).join('');
    const malformed = model.malformedReasons.length ? `<div class="hint">Malformed input: ${escapeText(model.malformedReasons.join(' · '))}</div>` : '<div class="hint">No malformed input reported.</div>';
    return `<section id="prototype-planning-inspection" class="panel"><h2>GDD prototype planning inspection</h2>${hint}<div class="field-grid"><div><strong>Status</strong><br>${escapeText(model.status)}</div><div><strong>Panels</strong><br>${escapeText(model.panels.length)}</div></div>${malformed}<ul>${rows}</ul><small>${escapeText(model.boundary)}</small></section>`;
  }


  function fullStudioEditorDemoModel(run) {
    const source = run?.full_studio_editor_demo || run?.fullStudioEditorDemo || null;
    ...
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        ...
        status: 'missing',
        issue: 774,
        panels: requiredPanelIds.map((id) => ({ id, label: id, required: true, present: false })),
        missingPanels: requiredPanelIds,
        forbiddenActions: ['trusted browser writes', 'command execution', 'publish/deploy/sign/upload', 'plugin execution', 'auto-apply', 'auto-merge'],
        boundary: 'Full Studio Editor integrated demo read model is unavailable; Studio remains read-only and draft-only.',
      };
    }
    const rawPanels = Array.isArray(source.panels) ? source.panels : [];
    const panelMap = new Map(rawPanels.map((panel) => [String(panel?.id || ''), panel]));
    ...
    const forbiddenActions = Array.isArray(source.forbidden_actions) ? source.forbidden_actions
      : Array.isArray(source.forbiddenActions) ? source.forbiddenActions : [];
    const generatedState = source.generated_state || source.generatedState || {};
    return {
      present: true,
      issue: source.issue || 774,
      status: source.status || (missingPanels.length ? 'incomplete' : 'ready'),
      fixtureRoot: source.fixture_root || source.fixtureRoot || 'examples/full-studio-editor-demo-v1',
      panels,
      missingPanels,
      draftPreviewRef: source.draft_preview_ref || source.draftPreviewRef || '',
      handoffPreviewRef: source.handoff_preview_ref || source.handoffPreviewRef || '',
      forbiddenActions,
      generatedState,
      boundary: source.boundary || 'Fixture-scoped Full Studio Editor demo; read-only inspection plus draft-only authoring and Safe Source Apply handoff preview.',
    };
  }

  function renderFullStudioEditorDemoSurface(run) {
    const model = fullStudioEditorDemoModel(run);
    const rows = model.panels.map((panel) => `<li class="surface-row"><strong>${escapeText(panel.label)}</strong> ${surfaceState(panel.present, panel.present ? 'included' : 'missing')}<br><small>${escapeText(panel.id)} · ${panel.required ? 'required' : 'optional'}</small></li>`).join('');
    const missing = model.missingPanels.length
      ? `<p class="warn">Missing required panels: ${escapeText(model.missingPanels.join(', '))}</p>`
      : '<p class="hint">All required integrated demo panels are represented by the fixture.</p>';
    const forbidden = model.forbiddenActions.length
      ? `<ul>${model.forbiddenActions.map((action) => `<li>${surfaceState(false, escapeText(action))}</li>`).join('')}</ul>`
      : '<p class="hint">No forbidden action list exported.</p>';
    const generated = model.generatedState && typeof model.generatedState === 'object'
      ? `<pre>${escapeText(JSON.stringify(model.generatedState, null, 2))}</pre>`
      : '<p class="hint">No generated-state summary exported.</p>';
    return `<section id="full-studio-editor-demo" class="panel"><h2>Full Studio Editor integrated demo</h2>
      <p class="hint">${escapeText(model.boundary)} This summary does not add write, command, publish/deploy, plugin execution, or apply controls.</p>
      <p>Issue #${escapeText(model.issue)} · fixture <code>${escapeText(model.fixtureRoot)}</code> · ${surfaceState(model.status === 'ready' && !model.missingPanels.length, model.status)}</p>
      ${missing}
      <h3>Required panels</h3><ul>${rows}</ul>
      <h3>Draft and handoff previews</h3><p><strong>Draft:</strong> ${escapeText(model.draftPreviewRef || 'not exported')}<br><strong>Safe Source Apply handoff:</strong> ${escapeText(model.handoffPreviewRef || 'not exported')}</p>
      <h3>Forbidden actions remain disabled</h3>${forbidden}
      <h3>Generated-state policy</h3>${generated}
    </section>`;
  }

  function renderEvidencePane(run) {

    return `${renderFullStudioEditorDemoSurface(run)}${renderStudioDiagnosticsSurface(run)}${renderProjectOverviewSurface(run)}${renderWorkspaceUxContractSurface()}${renderProjectWorkspaceSurface(run)}${renderProjectRunSurface(run)}${renderEvidenceFidelitySurface(run)}${renderEvidenceTimelineSurface(run)}${renderEvidenceBrowser(run)}${renderLiveObservabilityEvidenceSurface(run)}${renderStudioSourceGeneratedBrowserSurface(run)}${renderAuthoringProvenanceSurface(run)}${renderProvenanceAuditPanel(run)}${renderEngineExpansionSurface(run)}${renderStudio3dInspectionSurface(run)}${renderCameraLayerInspectionSurface(run)}${renderRenderBreakdownInspectionSurface(run)}${renderRuntimeProfilerInspectionSurface(run)}${renderRuntimeStateInspectionSurface(run)}${renderInputActionInspectionSurface(run)}${renderExpressiveComponentHudSurface(run)}${renderRuntimeEventInspectionSurface(run)}${renderRuntimeAssetLoadingSurface(run)}${renderAssetPreviewEvidenceSurface(run)}${renderPluginRegistryBrowserSurface(run)}${renderBehaviorEvidenceLifecycleSurface(run)}${renderSourceApplyWorktreeContextSurface(run)}${renderRouteAttemptEvidenceSurface(run)}${renderVisualComparisonEvidenceSurface(run)}${renderEvaluatorDepthInspectionSurface(run)}${renderPrototypePlanningInspectionSurface(run)}${renderStudioLevelDesignInspectionSurface(run)}${renderBehaviorDraftStatusSurface(run)}${renderBehaviorListPanel(run)}${renderBehaviorEventSignalPanel(run)}${renderBehaviorStateMachinePanel(run)}${renderBehaviorAbilityActionPanel(run)}${renderBehaviorReviewApplyStatusSurface(run)}${renderSourceApplyReviewSurface(run)}${renderExportInspectionSurface(run)}${renderSourcePatchEvidenceBundleSurface(run)}${renderSourcePatchApplyTransactionSurface(run)}${renderSourcePatchStaleTargetGuardSurface(run)}${renderStudioSceneEditorInteractionSpecSurface()}${renderStudioDraftAuthoringSurface(run)}${renderStudioDraftOperationModelSurface(run)}${renderStudioSourceApplyHandoffSurface(run)}${renderEntityComponentInspectorSurface(run)}${renderVisualDiffPreviewSurface(run)}${renderTilemapPainterSurface(run?.tilemap_painter || run?.tilemapPainter || run)}${renderTilemapDraftPreviewSurface(run)}${renderStudioAssetInspectorSurface(run)}${renderStudioPluginPanelSurface(run)}${renderStudioExportPackageInspectionSurface(run)}${renderStudioScenarioPanelSurface(run)}${renderStudioAssetBrowserSurface(run)}${renderJournalSurface(run)}${renderLoopDryRunSurface(run)}${renderLoopExecutionSurface(run)}${renderLoopRecoverySurface(run)}${renderStudioLoopCockpitSurface(run)}${renderLoopCoverageInspectionSurface(run)}${renderTrustGradientInspectionSurface(run)}${renderStudioMultiAgentPipelineInspectionSurface(run)}${renderProductionTaskBoardSurface(run)}${renderOwnershipPolicySurface(run)}${renderAgentRoleModelSurface(run)}${renderAgentWorkPackageSurface(run)}${renderQaSwarmInspectionSurface(run)}${renderQaAgentWorkQueueSurface(run)}${renderPerformanceRegressionLaneSurface(run)}${renderAgentHandoffSurface(run)}${renderReviewCriticGateSurface(run)}${renderProductionEvidenceBundleSurface(run)}${renderLoopEvidenceBundleSurface(run)}${renderMutationReviewSurface(run)}${renderRegressionPromotionSurface(run)}${renderRegressionMatrixSurface(run)}${renderReplaySurface(run)}${renderComparisonSurface(run)}`;
  }

  function renderIntegration(run, previewState = null) {
    return `${renderStudioNavigation(run)}${renderStudioAccessibilityNavSurface(run)}<section id="command-palette" class="panel">${renderStudioCommandPaletteSurface(run)}</section><section id="live-preview">${renderPreview()}<div id="preview-controls-host">${renderPreviewControls(previewState)}</div></section><section id="scene-editing">${renderQaPanel()}</section>${renderEvidencePane(run)}${renderStudioGaps()}`;
  }
  ...
  function studioSceneEditorInteractionSpec() {
    return {
      schemaVersion: 'studio-scene-editor-interaction-spec-v1',
      issue: '#2365',
      classification: 'contract-complete',
      draftTransactionModel: {
        baseModel: 'base-relative',
        baseDigestRequired: true,
        cumulativeSimulation: false,
        reference: 'docs/scene-only-mutation-v2.md',
        rule: 'Each scene draft validates against the base scene digest it declares; later drafts do not become a simulated new base until trusted Safe Source Apply records a reviewed source change.',
      },
      requiredSurfaces: [
        { id: 'scene-tree', hostSurface: 'renderStudioSceneTreeInspectorSurface', behavior: 'select authored/runtime nodes for context; no source writes' },
        { id: 'canvas-selection', hostSurface: 'renderStudioSceneCanvasSurface', behavior: 'visual selection and transform affordances are presentation/draft-only' },
        { id: 'transform-fields', hostSurface: 'studioCanvasTransformDraft', behavior: 'position/rotation/scale drafts validate bounds and base digest before handoff' },
        { id: 'component-inspector', hostSurface: 'renderEntityComponentInspectorSurface', behavior: 'allowlisted primitive fields only; unsupported fields visible read-only' },
        { id: 'diff-preview', hostSurface: 'studioDraftOperationPreviewDiff', behavior: 'review-only preview; blocked drafts produce no diff' },
        { id: 'apply-handoff', hostSurface: 'renderStudioSourceApplyHandoffSurface', behavior: 'Safe Source Apply handoff only; no browser trusted writes' },
        { id: 'accessibility', hostSurface: 'renderStudioAccessibilityNavSurface', behavior: 'keyboard landmarks and readable labels for core panes' },
      ],
      undoRedoPolicy: 'browser-memory drafts may be discarded/restored locally; trusted source undo/rollback is Safe Source Apply metadata, not cumulative browser simulation',
      forbiddenActions: ['direct_trusted_browser_write', 'cumulative_simulation_base', 'browser_command_bridge', 'self_approval', 'auto_apply'],
    };
  }

  function renderStudioSceneEditorInteractionSpecSurface() {
    const spec = studioSceneEditorInteractionSpec();
    const rows = spec.requiredSurfaces.map((surface) => `<li class="surface-row"><strong>${escapeText(surface.id)}</strong><br><small>${escapeText(surface.hostSurface)} · ${escapeText(surface.behavior)}</small></li>`).join('');
    return `<section id="scene-editor-interaction-spec" class="panel"><h2>Scene editor interaction spec</h2>
      <p class="hint">${escapeText(spec.schemaVersion)} · ${escapeText(spec.classification)}. Draft transaction base model: ${escapeText(spec.draftTransactionModel.baseModel)}; base digest required: ${escapeText(String(spec.draftTransactionModel.baseDigestRequired))}; cumulative simulation: ${escapeText(String(spec.draftTransactionModel.cumulativeSimulation))}.</p>
      <p class="hint">${escapeText(spec.draftTransactionModel.rule)}</p>
      <h3>Required surfaces</h3><ul>${rows}</ul>
      <p class="hint">Undo/redo: ${escapeText(spec.undoRedoPolicy)}. Forbidden: ${escapeText(spec.forbiddenActions.join(' · '))}.</p>
    </section>`;
  }
  ...
  const ASSET_BROWSER_ALLOWED_DESCRIPTOR_TYPES = ['asset-metadata', 'asset_metadata', 'texture-metadata', 'audio-metadata', 'model-metadata', 'generic-metadata'];

  function assetBrowserPathUnsafe(value) {
    const text = String(value == null ? '' : value);
    if (!text) return false;
    return /(^|[\\/])\.\.([\\/]|$)/.test(text) || text.startsWith('/') || /^[A-Za-z]:[\\/]/.test(text) || /[;&|`$<>]/.test(text);
  }

  function studioAssetBrowserModel(run) {
    const source = run?.asset_browser || run?.assetBrowser || null;
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        present: false,
        empty_state: source?.empty_state || source?.emptyState || 'No asset browser read model is available for this run.',
        boundary: 'Read-only asset manifest and plugin metadata inspection; Studio does not import, generate, upload, fetch, or mutate assets.',
        assets: [],
        types: [],
      };
    }
    ...
    const assets = rawAssets.map((asset, index) => {
      ...
      const sourcePath = asset?.source_path || asset?.sourcePath || '';
      const outputPath = asset?.output_path || asset?.outputPath || '';
      const status = asset?.status || 'unknown';
      const warnings = [];
      if (status === 'missing' || (!outputPath && status !== 'source-only')) warnings.push('Missing output artifact');
      if (asset?.hash && hashCounts[asset.hash] > 1) warnings.push('Duplicate asset hash');
      if (idCounts[id] > 1) warnings.push('Duplicate asset id');
      if (assetBrowserPathUnsafe(sourcePath)) warnings.push('Unsafe source path');
      if (assetBrowserPathUnsafe(outputPath)) warnings.push('Unsafe output path');
      const metadataRaw = Array.isArray(asset?.metadata) ? asset.metadata : (asset?.metadata ? [asset.metadata] : []);
      const metadata = metadataRaw.map((descriptor) => {
        const type = descriptor?.descriptor_type || descriptor?.descriptorType || descriptor?.type || 'unknown';
        const allowed = ASSET_BROWSER_ALLOWED_DESCRIPTOR_TYPES.includes(String(type));
        return {
          type: String(type),
          allowed,
          plugin: descriptor?.plugin || descriptor?.plugin_id || descriptor?.pluginId || 'unknown',
          fields: descriptor && typeof descriptor.fields === 'object' && !Array.isArray(descriptor.fields) ? descriptor.fields : {},
          blockedReason: allowed ? '' : `Descriptor type "${type}" is not allowlisted for read-only rendering.`,
        };
      });
      return {
        id,
    ...
    return {
      present: true,
      empty_state: source.empty_state || source.emptyState || 'No asset browser read model is available for this run.',
      boundary: source.boundary || 'Read-only asset manifest and plugin metadata inspection; Studio does not import, generate, upload, fetch, or mutate assets.',
      assets,
      types,
    };
  }

  function filterStudioAssets(assets, options) {
    ...
    return list.filter((asset) => {
      if (type && type !== 'all' && String(asset.type) !== type) return false;
      if (query && !`${asset.id} ${asset.type} ${asset.sourcePath} ${asset.outputPath}`.toLowerCase().includes(query)) return false;
      return true;
    });
  }

  function renderStudioAssetBrowserSurface(run) {
    const model = studioAssetBrowserModel(run);
    if (!model.present) {
      return `<section id="studio-asset-browser" class="panel"><h2>Asset browser</h2><p class="empty">${escapeText(model.empty_state)}</p><p class="hint">${escapeText(model.boundary)}</p></section>`;
    }
    const cards = model.assets.map((asset) => {
      const metaRows = asset.metadata.length
        ? asset.metadata.map((descriptor) => {
            if (!descriptor.allowed) return `<li>${surfaceState(false, escapeText(descriptor.blockedReason))}</li>`;
            const fieldText = Object.entries(descriptor.fields).map(([key, value]) => `${escapeText(key)}: ${escapeText(typeof value === 'object' ? JSON.stringify(value) : String(value))}`).join(' · ');
            return `<li>${surfaceState(true, escapeText(`${descriptor.type} (${descriptor.plugin})`))}<br><small>${fieldText || 'no fields'}</small></li>`;
          }).join('')
        : '<li>No metadata descriptors.</li>';
      const warnRows = asset.warnings.length
        ? `<ul class="asset-warnings">${asset.warnings.map((warning) => `<li>${surfaceState(false, escapeText(warning))}</li>`).join('')}</ul>`
        : '<p class="hint">No diagnostics.</p>';
      return `<article class="surface-row asset-card" data-asset-type="${escapeText(asset.type)}"><strong>${escapeText(asset.id)}</strong> <small>${escapeText(asset.type)} · ${escapeText(asset.status)}</small>
        <br><small>source ${escapeText(asset.sourcePath)} → output ${escapeText(asset.outputPath)}</small>
        <br><small>hash ${escapeText(asset.hash)}${asset.size != null ? ` · ${escapeText(String(asset.size))} bytes` : ''}</small>
        ${warnRows}
        <h4>Plugin metadata (read-only)</h4><ul>${metaRows}</ul>
      </article>`;
    }).join('') || '<p class="empty compact">No assets recorded.</p>';
    const filterNote = model.types.length ? `Types: ${model.types.join(', ')}. Use filterStudioAssets({ type, query }) for read-only filtering/search.` : 'No asset types recorded.';
    return `<section id="studio-asset-browser" class="panel"><h2>Asset browser</h2>
      <p class="hint">${escapeText(model.boundary)} This surface inspects asset manifests and plugin-provided metadata read-only. It does not import, generate, upload, fetch remote assets, write manifests, run commands, or apply edits.</p>
      <p class="hint">${escapeText(filterNote)}</p>
      ${cards}
    </section>`;
  }

  function studioScenarioPanelModel(run) {
    const source = run?.scenario_panel || run?.scenarioPanel || null;
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        present: false,
        empty_state: source?.empty_state || source?.emptyState || 'No scenario/playtest read model is available for this run.',
        boundary: 'Read-only scenario/playtest evidence inspection; Studio does not launch runs, start scenarios, or execute commands.',
        scenarios: [],
        runControlsEnabled: false,
      };
    }
    const scenarios = Array.isArray(source.scenarios) ? source.scenarios.map((scenario, index) => {
      ...
      const evidenceLinks = Array.isArray(scenario?.evidence_links) ? scenario.evidence_links
        : Array.isArray(scenario?.evidenceLinks) ? scenario.evidenceLinks : [];
      const screenshots = Array.isArray(scenario?.screenshots) ? scenario.screenshots : [];
      const logs = Array.isArray(scenario?.logs) ? scenario.logs : [];
      const warnings = [];
      const stale = scenario?.stale === true || scenario?.evidence_stale === true || scenario?.evidenceStale === true;
      if (stale) warnings.push('Stale evidence: re-run required to refresh artifacts');
      if (!evidenceLinks.length && verdict && verdict !== 'unknown') warnings.push('No evidence links recorded for a completed verdict');
      const brokenEvidence = evidenceLinks.filter((link) => typeof link === 'object' && (link.missing === true || link.broken === true));
      if (brokenEvidence.length) warnings.push(`${brokenEvidence.length} broken/missing evidence reference(s)`);
      return {
        id: scenario?.id || scenario?.scenario_id || `scenario-${index + 1}`,
        templateSource: scenario?.template_source || scenario?.templateSource || 'unknown',
        runStatus: status,
        verdict,
        failureClassification: scenario?.failure_classification || scenario?.failureClassification || (String(verdict).toLowerCase() === 'fail' || String(verdict).toLowerCase() === 'failed' ? 'unclassified-failure' : ''),
        screenshots,
        worldState: scenario?.world_state || scenario?.worldState || null,
        logs,
        evidenceLinks: evidenceLinks.map((link) => (typeof link === 'string' ? { ref: link } : (link && typeof link === 'object' ? link : { ref: 'unknown' }))),
        stale,
        warnings,
      };
    }) : [];
    return {
      present: true,
      empty_state: source.empty_state || source.emptyState || 'No scenario/playtest read model is available for this run.',
      boundary: source.boundary || 'Read-only scenario/playtest evidence inspection; Studio does not launch runs, start scenarios, or execute commands.',
      scenarios,
      runControlsEnabled: false,
    };
  }

  function renderStudioScenarioPanelSurface(run) {
    const model = studioScenarioPanelModel(run);
    if (!model.present) {
      return `<section id="studio-scenario-panel" class="panel"><h2>Scenario and playtest panel</h2><p class="empty">${escapeText(model.empty_state)}</p><p class="hint">${escapeText(model.boundary)}</p></section>`;
    }
    const rows = model.scenarios.map((scenario) => {
      const passLike = /^(pass|passed|ok|green)$/i.test(String(scenario.verdict));
      const evidenceRows = scenario.evidenceLinks.length
        ? scenario.evidenceLinks.map((link) => {
            const broken = link.missing === true || link.broken === true;
            return `<li>${surfaceState(!broken, escapeText(link.ref || link.path || 'unknown'))}${broken ? ' (broken/missing)' : ''}</li>`;
          }).join('')
        : '<li>No evidence links recorded.</li>';
      const failureDetail = scenario.failureClassification
        ? `<p class="warn">Failure classification: ${escapeText(scenario.failureClassification)}</p>`
        : '';
      const warnRows = scenario.warnings.length
        ? `<ul class="scenario-warnings">${scenario.warnings.map((warning) => `<li>${surfaceState(false, escapeText(warning))}</li>`).join('')}</ul>`
        : '<p class="hint">No diagnostics.</p>';
      const logRows = scenario.logs.length
        ? `<ul>${scenario.logs.slice(0, 5).map((log) => `<li><small>${escapeText(typeof log === 'string' ? log : JSON.stringify(log))}</small></li>`).join('')}</ul>`
        : '';
      return `<article class="surface-row scenario-row" data-scenario="${escapeText(scenario.id)}"><strong>${escapeText(scenario.id)}</strong> ${surfaceState(passLike, scenario.verdict)} <small>${escapeText(scenario.runStatus)} · template ${escapeText(scenario.templateSource)}</small>
        ${failureDetail}
        ${warnRows}
        <h4>Evidence links (read-only)</h4><ul>${evidenceRows}</ul>
        ${scenario.screenshots.length ? `<p class="hint">${escapeText(String(scenario.screenshots.length))} screenshot reference(s) recorded.</p>` : ''}
        ${logRows}
      </article>`;
    }).join('') || '<p class="empty compact">No scenarios recorded.</p>';
    return `<section id="studio-scenario-panel" class="panel"><h2>Scenario and playtest panel</h2>
      <p class="hint">${escapeText(model.boundary)} This panel inspects existing scenario/playtest/evidence data only. Run controls: ${surfaceState(false, 'disabled · no autonomous run/start')}. It does not launch runs, start scenarios, run commands, or orchestrate QA beyond existing allowlisted trusted-runner paths.</p>
      ${rows}
    </section>`;
  }

  function studioExportPackageInspectionModel(run) {
    const source = run?.export_package_panel || run?.exportPackagePanel || null;
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        present: false,
        empty_state: source?.empty_state || source?.emptyState || 'No export/package inspection read model is available for this run.',
        boundary: 'Read-only export/package evidence inspection; Studio does not execute export, publish, deploy, sign, upload, or arbitrary commands.',
        profile: '',
        plan: [],
    ...
    const artifacts = Array.isArray(source.artifacts) ? source.artifacts.map((artifact, index) => ({
      name: artifact?.name || artifact?.path || `artifact-${index + 1}`,
      checksum: artifact?.checksum || artifact?.hash || 'unknown',
      size: typeof artifact?.size === 'number' ? artifact.size : null,
    ...
    return {
      present: true,
      empty_state: source.empty_state || source.emptyState || 'No export/package inspection read model is available for this run.',
      boundary: source.boundary || 'Read-only export/package evidence inspection; Studio does not execute export, publish, deploy, sign, upload, or arbitrary commands.',
      profile: source.profile || source.export_profile || source.exportProfile || 'unknown',
      plan,
      packageStatus: source.package_status || source.packageStatus || 'unknown',
      artifacts,
      verificationVerdict: source.verification_verdict || source.verificationVerdict || 'unknown',
      warnings,
      publishBlocked: source.publish_blocked === false ? false : true,
    };
  }

  function renderStudioExportPackageInspectionSurface(run) {
    const model = studioExportPackageInspectionModel(run);
    if (!model.present) {
    ...
    const artifactRows = model.artifacts.length
      ...
      : '<li class="surface-row">No artifacts recorded.</li>';
    const warnRows = model.warnings.length
      ? `<ul class="export-warnings">${model.warnings.map((warning) => `<li>${surfaceState(false, escapeText(typeof warning === 'string' ? warning : JSON.stringify(warning)))}</li>`).join('')}</ul>`
      : '<p class="hint">No generated-state warnings.</p>';
    const verdictPass = /^(pass|passed|verified|ok|green)$/i.test(String(model.verificationVerdict));
    return `<section id="studio-export-package-panel" class="panel"><h2>Export / package inspection</h2>
      <p class="hint">${escapeText(model.boundary)} This panel consumes existing export evidence and package descriptors read-only. Execution stays in trusted CLI/harness paths. Export/publish/deploy/sign/upload: ${surfaceState(false, 'disabled · no execution from Studio')}.</p>
      <p>Profile: <strong>${escapeText(model.profile)}</strong> · Package status: ${surfaceState(model.packageStatus === 'ready' || model.packageStatus === 'ok', model.packageStatus)}</p>
      <p>Verification verdict: ${surfaceState(verdictPass, model.verificationVerdict)}</p>
      <p>Publish/release: ${surfaceState(false, model.publishBlocked ? 'blocked · publish/deploy not available from Studio' : 'reported as allowed by trusted evidence (Studio still cannot execute it)')}</p>
      <h4>Export plan (read-only)</h4>${planRows}
      <h4>Artifacts and checksums</h4><ul>${artifactRows}</ul>
      <h4>Generated-state warnings</h4>${warnRows}
    </section>`;
  }

  const PLUGIN_PANEL_ALLOWED_DESCRIPTOR_TYPES = ['info-panel', 'metadata-panel', 'readonly-table', 'key-value-panel', 'text-panel'];

  function studioPluginPanelModel(run) {
    const source = run?.plugin_panel || run?.pluginPanel || null;
    if (!source || typeof source !== 'object' || Array.isArray(source) || source.present === false) {
      return {
        present: false,
        empty_state: source?.empty_state || source?.emptyState || 'No plugin panel read model is available for this run.',
        boundary: 'Read-only plugin registry and descriptor inspection; Studio does not install, update, delete, enable, or run plugins.',
        plugins: [],
        allowedDescriptorTypes: PLUGIN_PANEL_ALLOWED_DESCRIPTOR_TYPES.slice(),
      };
    }
    const plugins = Array.isArray(source.plugins) ? source.plugins.map((plugin, index) => {
      const capabilities = Array.isArray(plugin?.capabilities) ? plugin.capabilities : [];
      const extensionPoints = Array.isArray(plugin?.extension_points) ? plugin.extension_points
        : Array.isArray(plugin?.extensionPoints) ? plugin.extensionPoints : [];
      const blockedReasons = Array.isArray(plugin?.blocked_reasons) ? plugin.blocked_reasons
        : Array.isArray(plugin?.blockedReasons) ? plugin.blockedReasons : [];
      const contributedRaw = Array.isArray(plugin?.contributed_panels) ? plugin.contributed_panels
        : Array.isArray(plugin?.contributedPanels) ? plugin.contributedPanels : [];
      const contributedPanels = contributedRaw.map((panel) => {
        const type = String(panel?.descriptor_type || panel?.descriptorType || panel?.type || 'unknown');
        const allowed = PLUGIN_PANEL_ALLOWED_DESCRIPTOR_TYPES.includes(type);
        return {
          type,
          allowed,
          title: panel?.title || panel?.label || 'Untitled panel',
          fields: panel && typeof panel.fields === 'object' && !Array.isArray(panel.fields) ? panel.fields : {},
          text: typeof panel?.text === 'string' ? panel.text : '',
          blockedReason: allowed ? '' : `Descriptor panel type "${type}" is not allowlisted for read-only rendering.`,
        };
      });
      return {
        id: plugin?.id || plugin?.plugin_id || `plugin-${index + 1}`,
        name: plugin?.name || plugin?.id || `Plugin ${index + 1}`,
        validationStatus: plugin?.validation_status || plugin?.validationStatus || 'unknown',
        compatible: plugin?.compatible === true,
        compatibility: plugin?.compatibility || (plugin?.compatible === true ? 'compatible' : 'unknown'),
        capabilities,
        extensionPoints,
        blockedReasons,
        contributedPanels,

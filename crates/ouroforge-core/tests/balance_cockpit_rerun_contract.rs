//! Contract test for Balance Cockpit Read-Only Surface and Re-Run Diff v1
//! (#1608).
//!
//! Trusted Rust mirror of the runtime balance cockpit
//! `examples/game-runtime/balance-cockpit.js` and its runtime test
//! `examples/game-runtime/balance-cockpit.test.cjs`. Per the Era F language
//! boundary, the trusted re-run/diff logic is owned by Rust/local; the
//! JavaScript module reproduces the same observable diff for the browser-local
//! read-only cockpit. Both sides use integer-only aggregation over the same
//! persona decision stream (#1606), so the re-run diff is digest-identical
//! across the two languages.
//!
//! The cockpit is read-only and human-in-the-loop: a proposed balance change
//! (e.g. a nerf) is applied to a COPY of the deck spec, the identical seed
//! distribution is re-run, and the win-rate impact is diffed — never an
//! auto-applied nerf and never a trusted write. The diff reuses the compare
//! digest-equality signal and mirrors the compare evidence shape. The test
//! machine-checks: a nerf re-run diffs the win-rate, the diff is deterministic,
//! the proposed change never mutates the trusted spec, surfacing is HTML-escaped
//! (no execution) with per-flag counterexamples, and malformed input fails
//! closed.
//!
//! The deck-roguelike, persona, and telemetry-aggregation machinery below
//! mirrors `balance_telemetry_contract.rs` (#1607); this test adds the trusted
//! re-run + diff and the read-only surfacing.
...
const RERUN_SCENE: &str = "examples/game-runtime/deck-roguelike-balance-rerun-scene-v1.json";
const CHANGE_FIXTURE: &str = "examples/game-runtime/balance-rerun-change-v1.json";

// Pinned re-run diff digest on the planted scene (seed 31). Identical to the
// string the JS runtime test asserts, so the trusted Rust mirror and the
// browser-local cockpit agree byte-for-byte on the re-run diff.
const EXPECTED_DIFF_DIGEST: &str = "rerun-diff|status=changed|change=smite[damage=6]|wr=5/5->2/5(-3)|cards=brick:0->0,defend:1->3,smite:20->38,strike:5->14|degenResolved=|degenIntroduced=|deadResolved=|deadIntroduced=";
...
// --- Balance cockpit: read-only surfacing + seeded re-run diff -----------------
...
#[test]
fn nerf_rerun_diffs_the_win_rate_and_is_deterministic() {
    let spec = deck_spec(RERUN_SCENE);
    ...
    assert_eq!(diff.wr_after, (2, 5), "the nerf drops the win-rate");
    assert_eq!(diff.delta_wins, -3, "the win-rate impact is diffed");
    assert_eq!(
...
#[test]
fn proposed_change_never_mutates_the_trusted_spec() {
    let spec = deck_spec(RERUN_SCENE);
    let change = read_json(CHANGE_FIXTURE)["change"].clone();
    let candidate = apply_balance_change(&spec, &change).expect("valid change");
    assert_eq!(
        candidate["cards"]["smite"]["damage"].as_u64(),
        Some(6),
        "change applied to the copy"
    );
    assert_eq!(
...
#[test]
fn surfacing_is_escaped_and_carries_per_flag_counterexamples() {
    // A hostile scene id must be escaped, never executed.
    assert_eq!(
        escape_text("<script>alert(1)</script>"),
        "&lt;script&gt;alert(1)&lt;/script&gt;",
        "surfacing HTML-escapes, never executes"
    );
    // Each flag carries a replayable counterexample seed.
    let report = aggregate_spec(&deck_spec(RERUN_SCENE));
    assert!(
        report
            .degenerate_combos
            .iter()
            .all(|g| g.replay_deck_seed == 31),
        "degenerate flags carry the replay seed"
    );
    assert!(
        report.dead_items.iter().any(|d| d.card == "brick"),
        "the dead item is surfaced"
    );
}

#[test]
fn malformed_change_fails_closed() {
    let spec = deck_spec(RERUN_SCENE);
    assert!(
        apply_balance_change(&spec, &serde_json::json!({"card": "phantom", "damage": 1}))
            .unwrap_err()
            .contains("undeclared card \"phantom\""),
        "an unknown card fails closed"
    );
    assert!(
        apply_balance_change(&spec, &serde_json::json!({"card": "smite"}))
            .unwrap_err()
            .contains("must set at least one of"),
        "a no-op change fails closed"
    );
    assert!(
        apply_balance_change(&spec, &serde_json::json!({"card": "smite", "damage": -1}))
            .unwrap_err()
            .contains("must be a non-negative integer"),
        "a negative field fails closed"
    );
}

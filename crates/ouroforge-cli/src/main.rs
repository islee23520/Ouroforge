...
#[derive(Debug, Subcommand)]
enum DogfoodCommand {
    /// Create a guided local grid-puzzle dogfood project without raw JSON/YAML authoring.
    GuidedGridPuzzle {
        destination: PathBuf,
        #[arg(long, default_value = "crate-garden-guided-2512")]
        project_id: String,
        #[arg(long, default_value = "Crate Garden Guided")]
        title: String,
        #[arg(long, default_value_t = 2512)]
        issue: u64,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Dogfood {
            command:
                DogfoodCommand::GuidedGridPuzzle {
                    destination,
                    project_id,
                    title,
                    issue,
            ...
            let summary =
                write_guided_grid_puzzle_project(&destination, &project_id, &title, issue)?;
            println!("{}", serde_json::to_string_pretty(&summary)?);
        }
        Commands::Dogfood {
            command:
...
fn write_guided_grid_puzzle_project(
    destination: &Path,
    project_id: &str,
    title: &str,
    issue: u64,
    ...
    {
        return Err(anyhow!(
            "guided grid-puzzle project id must be non-empty ASCII alphanumeric/dash/underscore"
        ));
    }
    if title.trim().is_empty() {
        return Err(anyhow!("guided grid-puzzle title must not be empty"));
    }
    ...
    for dir in dirs {
        std::fs::create_dir_all(destination.join(dir))
            .with_context(|| format!("failed to create {}", destination.join(dir).display()))?;
    }

    let seed_path = format!("seeds/{project_id}.yaml");
    let before_scene_path = "scenes/before-review.scene.json".to_string();
    let after_scene_path = format!("scenes/{project_id}.scene.json");
    let scenario_path = format!("scenarios/{project_id}-core.json");
    let review_path = "review/review-apply-decision.json".to_string();
    let playtest_path = "playtest/playtest-backlog.json".to_string();

    let seed = format!(
        "id: {project_id}\ntitle: {title}\ngoal: Generate and validate a local guided grid-puzzle project without raw JSON/YAML hand editing.\nconstraints:\n  target: game-runtime\nacceptance:\n  - Project manifest, seed, scenario, review/apply decision, playtest backlog, and scenes are generated by the guided CLI command.\n  - The after-review scene replays the intended grid-puzzle solution to grid_won=true in the local runtime.\n  - Browser and Studio surfaces remain read-only; trusted writes stay in the Rust/local CLI gate.\nscenarios:\n  - id: guided-grid-puzzle-win-path\n    description: Replay the generated grid-puzzle intended solution and observe the win state.\n    assertions:\n      - world_state:\n          path: gridPuzzle.status\n          equals: won\n      - world_state:\n          path: gridPuzzle.win.satisfied\n          equals: true\n",
        project_id = project_id,
        title = title
    );

    let before_scene = grid_scene_json(
        project_id,
        title,
        issue,
        "before-review",
        "intendedSolution stops before covering the target",
        &["left", "left"],
    );
    let after_scene = grid_scene_json(
        project_id,
        title,
        issue,
        "after-review-apply",
        "none for scoped mechanical puzzle loop",
        &["left", "down", "left", "up"],
    );

    let project_manifest = serde_json::json!({
        "schemaVersion": "project-manifest-v1",
        "project": { "id": project_id, "name": title },
        "scenes": [
            { "id": "before-review", "path": before_scene_path },
            { "id": "main", "path": after_scene_path }
        ],
        "seeds": [{ "id": project_id, "path": seed_path }],
        "scenarioPacks": [{ "id": format!("{project_id}-core"), "path": scenario_path }],
        "assetRoots": ["assets"],
        "runsRoot": "runs",
    ...
    let scenario = serde_json::json!({
        ...
        "id": format!("{project_id}-core"),
        "description": "Guided CLI generated grid-puzzle scenario pack for #2512.",
        "seed": seed_path,
        "scenes": [after_scene_path],
        "scenarioGroups": [{
            "id": "guided-grid-puzzle-core",
            "description": "Mechanical grid-puzzle objective from guided authoring artifacts.",
            "scenarios": [{
                "id": "guided-grid-puzzle-win-path",
                "description": "Replay the guided intended solution and verify grid_won=true.",
                "assertions": [
                    {"world_state": {"path": "gridPuzzle.status", "equals": "won"}},
                    {"world_state": {"path": "gridPuzzle.win.satisfied", "equals": true}}
                ]
            }]
        }]
    });
    ...
    let review = serde_json::json!({
        "schemaVersion": "ouroforge.review-apply-decision.v1",
        "issue": issue,
        "decisionId": format!("{project_id}-guided-review-apply"),
        "guidedAuthoringCommand": "ouroforge dogfood guided-grid-puzzle",
        "proposal": "Replace incomplete before-review intendedSolution with the reviewed four-move solution while keeping the guided level layout unchanged.",
        "beforeScene": before_scene_path,
        "afterScene": after_scene_path,
        "reviewer": { "type": "human-required-template", "selfApproval": false },
        "trustedWriteBoundary": {
            "browserTrustedWrite": false,
            "commandBridge": false,
            "autoApply": false,
            "autoMerge": false
        },
        "rawJsonManualSteps": [],
        "rawFileEditingRequired": false,
        "guidedCliGenerated": true
    });

    let playtest = serde_json::json!({
        "schemaVersion": "playtest-gap-backlog-v1",
        "issue": issue,
        "backlogId": format!("{project_id}-playtest-guided"),
        "findings": [],
        "readModel": {
            "findingCount": 0,
            "blockingDeferredCount": 0,
            "openNonBlockingCount": 0,
            "futureProposalCandidateCount": 0,
            "closureAllowed": true
        },
        "guidedAuthoringCommand": "ouroforge dogfood guided-grid-puzzle",
        "rawFileEditingRequired": false,
        "humanFunFeelJudgment": "not claimed"
    });

    write_text(destination.join(&seed_path), &seed)?;
    write_json(
        destination.join("ouroforge.project.json"),
        &project_manifest,
    )?;
    write_json(destination.join(&before_scene_path), &before_scene)?;
    write_json(destination.join(&after_scene_path), &after_scene)?;
    write_json(destination.join(&scenario_path), &scenario)?;
    write_json(destination.join(&review_path), &review)?;
    write_json(destination.join(&playtest_path), &playtest)?;
    write_text(
        destination.join("assets/README.md"),
        "# Guided grid-puzzle assets\n\nNo external assets are required for this local grid-puzzle fixture.\n",
    )?;

    Ok(serde_json::json!({
        "schemaVersion": "guided-grid-puzzle-authoring-v1",
        "issue": issue,
        ...
        "artifacts": {
            "seed": seed_path,
            "projectManifest": "ouroforge.project.json",
            "beforeScene": before_scene_path,
            "afterScene": after_scene_path,
            "scenarioPack": scenario_path,
            "reviewApplyDecision": review_path,
            "playtestBacklog": playtest_path
        },
        "rawFileEditingRequired": false,
        "guardrails": {
            "browserTrustedWrite": false,
            ...
            "signingUploadStoreExportExpansion": false,
            "godotParityClaim": false
        }
    }))
}
...
fn grid_scene_json(
    ...
    serde_json::json!({
        "schemaVersion": "1",
        "id": if state == "before-review" { format!("{project_id}-before-review") } else { project_id.to_string() },
        "bounds": { "width": 192, "height": 160 },
        "renderer": { "background": "#18212f", "viewport": { "width": 192, "height": 160 } },
        "entities": [],
        "metadata": {
            "issue": issue,
            "title": title,
